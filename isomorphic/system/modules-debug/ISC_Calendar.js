...
if (window.isc && isc.version != "v14.1p_2025-06-14/EVAL Development Only" && !isc.DevUtil) {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'v14.1p_2025-06-14/EVAL Development Only'. Mixing resources from different "
...
isc.CalendarView.addProperties({
    ...
    // clear hover-sizes - if still unset at runtime, they'll use the Calendar's settings
    hoverAutoFitWidth: null,
    hoverAutoFitMaxWidth: null,

    // events can span multiple days visually in vertical-views and timelines
    ...
    init : function () {
        ...
        // if hover auto-sizing settings are null, use the parent Calendar's settings
        if (this.hoverAutoFitWidth == null) {
            this.hoverAutoFitWidth = this.calendar.hoverAutoFitWidth;
        }
        if (this.hoverAutoFitMaxWidth == null) {
            this.hoverAutoFitMaxWidth = this.calendar.hoverAutoFitMaxWidth;
        }
    ...
    // helper to determine whether to refresh events when a view draws
    shouldRefreshEventsOnDraw : function () {
        var cal = this.calendar,
            localData = !cal.getDataSource(),
            autoFetch = !localData && cal.autoFetchData,
            pendingFetch = cal._pendingFetchAfterDraw,
            parentDrawn = cal._doneInitialDraw,
        ...
        // we want to refresh events on draw() if the data is local, or if there's a DS and
        // autoFetchData is set, and the parent is either drawn or currently in animateShow()
        //return localData ||
        //        ((autoFetch || pendingFetch) && (parentDrawn || parentAnimating));

        return localData || pendingFetch ||
                (autoFetch  && (parentDrawn || parentAnimating));
    },

    longEventsLayoutDefaults: {
        _constructor: "Canvas",
        width: "100%",
        overflow: "visible",
        minHeight: 10,
        ...
        // list of child-canvases created by getChildCanvas()
        canvasList: null,

        // list of created child-canvases not currently being used
        canvasPool: null,

        // list of event-data to render canvases for
        eventList: null,

        isLongEventsLayout: true,

        // show the day when hovering over the longEventsLayout
        canHover: true,
        showHover: true,
        getHoverHTML : function () {
            return this.calendar.getLongEventLayoutHoverHTML(this, this.calendarView);
        },

        // start and end of the range - these come from the currentRecord when used as
        // recordComponents in the monthView, and from the view-range otherwise
        getStartDate : function () {
            if (this.currentRecord) return this.currentRecord.startDate;
            return this.calendarView.startDate;
        },
        getEndDate : function () {
            if (this.currentRecord) return this.currentRecord.endDate;
            return this.calendarView.endDate;
        },

        // the ghost event is used to show an event being drag-moved or drag-created
        // - in the monthView, the drag might start in a different longEventsLayout
        // but the ghost-event might show in multiple layouts if it spans weeks
        ghostEvent: { name: "ghost" },
        setGhostCanvas : function (canvas) {
            if (!this.ghostCanvas) {
                this.ghostCanvas = this.createAutoChild("canvas", {
                    height: this.canvasHeight,
                    calendarView: this.creator,
                    calendar: this.creator.creator,
                    isGhost: true,
                    opacity: this.calendar.longEventDragOpacity,
                    eventProxy: this
                });
                this.addChild(this.ghostCanvas);
            }
            if (canvas) {
                this.ghostCanvas.setStyleName(canvas.styleName);
                this.ghostEvent[this.calendar.startDateField] = canvas.event[this.calendar.startDateField];
                this.ghostEvent[this.calendar.endDateField] = canvas.event[this.calendar.endDateField];
            } else if (this.calendarView.ghostEvent) {
                var ge = this.calendarView.ghostEvent;
                this.ghostEvent[this.calendar.startDateField] = ge[this.calendar.startDateField];
                this.ghostEvent[this.calendar.endDateField] = ge[this.calendar.endDateField];
            }

        },
        updateGhostCanvas : function (startDate, endDate) {
            if (this.ghostDrag) {
                var dates = this.getMouseDates();
                startDate = startDate || dates[0];
                endDate = endDate || dates[1];
            } else {
                startDate = startDate || this.ghostEvent[this.calendar.startDateField];
                endDate = endDate || this.ghostEvent[this.calendar.endDateField];
            }

            this.ghostEvent[this.calendar.startDateField] = startDate;
            this.ghostEvent[this.calendar.endDateField] = endDate;

            this.ghostCanvas.setEvent(this.ghostEvent);
            if (startDate > this.endDate || endDate < this.startDate) {
                this.ghostCanvas.hide();
                //this.ghostCanvas.resizeTo(0);
            } else {
                this.ghostCanvas.moveTo(this.getDateLeft(this.ghostEvent.startDate), 0);
                this.ghostCanvas.resizeTo(this.getEventWidth(this.ghostEvent));
                if (!this.ghostCanvas.isVisible()) this.ghostCanvas.show();
                this.ghostCanvas.updateEndStyles();
            }
            if (this.ghostCanvas.isVisible()) this.ghostCanvas.bringToFront();
        },

        showGhostCanvas : function (externalCanvas) {
            this.setGhostCanvas(externalCanvas);
            this.updateGhostCanvas();

            this.ghostCanvas.setEvent(this.ghostEvent);

            // if the dates of the ghost event overlap with this longEventLayout, show the
            // portion of the event that overlaps
            var inRange = this.ghostEvent[this.calendar.startDateField] < this.endDate &&
                this.ghostEvent[this.calendar.endDateField] > this.startDate;
            if (inRange) {
                if (!this.ghostCanvas.isVisible()) this.ghostCanvas.show();
                this.ghostCanvas.bringToFront();
            }
        },

        hideGhostCanvas : function () {
            this.ghostCanvas && this.ghostCanvas.hide();
        },

        handleMouseDown : function () {
            //isc.logWarn("in layout.handleMouseDown");
            this.ghostDrag = true;
            this.showGhostCanvas();
        },

        handleMouseUp : function () {
            //isc.logWarn("in layout.handleMouseUp");
            //this.hideGhostCanvas();
            if (this.ghostDrag) {
                delete this.ghostDrag;
                // the drag was started on this longEventLayout - need to create a new event
                var evt = isc.addProperties({}, this.ghostEvent);
                // tweak the dates = we want a one-hour event, which is also flagged as allDay
                evt[this.calendar.startDateField].setHours(new Date().getHours());
                evt[this.calendar.endDateField].setHours(evt[this.calendar.startDateField].getHours() + 1);
                evt[this.calendar.allDayField] = true;
                evt.name = "New Event";
                this.calendar.showEventDialog(evt, true);
            }
            return false;
        },

        // deal with drag-move functionality
        handleMouseMove : function () {
            //isc.logWarn("in layout.handleMouseMove");
            var dates = this.getMouseDates(),
                startDate = this.ghostEvent[this.calendar.startDateField],
                endDate = this.ghostEvent[this.calendar.endDateField]
            ;

            if (this.ghostDrag) {
                // if a drag was started on this layout, we're drag-creating a new event - update
                // the ghostCanvas to reflect the drag-dates

                // dragged left into an earlier day
                if (dates[0] < startDate) startDate = dates[0];
                // dragged right into a later day
                else endDate = dates[1];

                this.updateGhostCanvas(startDate, endDate)
            }
        },

        canvasDefaults: {
            _constructor: "EventCanvas",
            overflow: "hidden",
            ...
            headerLabelProperties: { overflow: "hidden" },

            showBody: false,
            //showHeader: false,
            getHeaderHTML : function () {
                var result = this.calendar.getLongEventHTML(this.event, this.creator, this.calendarView);
                return result;
            },

            getHoverHTML : function () {
                var result = this.calendar.getLongEventHoverHTML(this.event, this.creator, this.calendarView);
                return result;
            },

            click : function () {
                return this.creator.eventClick(this);
            },

            isLongEvent: true,

            // Keep the original canvas in place
            canDrag: true,
            canDragReposition: true,
            dragAppearance: "none",
            //dragIntersectStyle: "rect",
            ...
            dragRepositionStart : function () {
                this.mouseDownDates = this.creator.getMouseDates();
                //this.calendarView.showGhostCanvas(this, this.mouseDownDates[0], this.mouseDownDates[1]);
                this.calendarView.startGhostDragMove(this);

                // this could do with a second look later - "false" doesn't work in vertical views
                return !this.calendarView.isMonthView();
            },

            dragRepositionMove : function () {
                var dates = this.creator.getMouseDates();

                // bail if no cell with valid dates under the mouse
                if (!dates || !dates[0] || !dates[1]) return;

                var view = this.calendarView;

                // mouse in a different column - update the ghostCanvas dates on the view
                this.calendarView.updateGhostCanvas(dates[0], dates[1], "canvasDragRepositionMove");
            },

            dragRepositionStop : function () {
                // hide the ghostCanvas first
                var view = this.calendarView;
                view.hideGhostCanvas();
                // see if the dates have changed
                var dates = this.creator.getMouseDates();
                if (!dates) return;
                // same mouse-dates, nothing to do
                if (dates[0] == this.mouseDownDates[0]) return;
                // update the event
                var evt = view.ghostEvent;
                this.calendar.updateCalendarEvent(view.ghostCanvas.event, evt);
                //return false;
            },

            draw : function () {
                var result = this.Super("draw", arguments);
                // update the secondary end-styles that show when the event extends beyond the
                // range at either end
                this.updateEndStyles();
            },

            getHeaderHeight : function () {
                return this.calendar && this.calendar.longEventHeight;
            },

            setStyleName : function () {
                var result = this.Super("setStyleName", arguments);
                this.updateEndStyles();
                return result;
            },

            updateEndStyles : function () {
                if (!this.isDrawn() || !this.event) return;
                var cal = this.calendar,
                    view = this.calendarView,
                    evt = this.event,
                    layout = this.creator,
                    // check if the event extends beyond the view-range - if it does,
                    // show special styling on the ends of this event-canvas
                    styleLeft = evt[cal.startDateField].getTime() < view.startDate.getTime(),
                    styleRight = evt[cal.endDateField].getTime() > view.endDate.getTime()
                ;

                var element = this.getPartHandle(),
                    header = this.getShowHeader() ? this.getPartHandle("header") : null
                ;
                if (!header) {
                    this.logWarn("No header-element in updateEndStyles()");
                    return;
                }

                // this._currentRowNum is set when this canvas is one of several in the
                // monthView - only style the left end of the first one and the right end
                // of the last visible one, the extremes of the view-range
                if (layout._currentRowNum != null) {
                    if (styleLeft) styleLeft = layout._currentRowNum == 1;
                    if (styleRight) styleRight = layout._currentRowNum == view.data.length - 1;
                }

                var style = null;

                // apply a single style that affects one or both ends - the builtin style
                // uses CSS clip-paths and you can only apply one of those to an element
                if (styleLeft && styleRight) style = this.baseStyle + "BothEnds";
                else if (styleLeft) style = this.baseStyle + "LeftEnd";
                else if (styleRight) style = this.baseStyle + "RightEnd";

                if (style) {
                    // apply the clipping-style to both the canvas and the header-label
                    element.classList.add(style);
                    if (header) header.classList.add(style);
                } else this.clearEndStyles();

                this._currentEndStyle = style;

                var handle = this.getStyleHandle();
                if (handle) {
                    handle["border"] = "none";
                    handle["outline"] = "none";
                    //handle = this.headerLabel && this.headerLabel.getHandle();
                    handle = this.headerLabel.getStyleHandle();
                    if (handle) {
                        handle["padding-top"] = "0px";
                        handle["padding-bottom"] = "0px";
                        handle["border"] = "none";
                        handle["outline"] = "none";
                    }
                }

                isc.logInfo("New End style for event '" + this.event[cal.nameField] + "': " +
                    (this._currentEndStyle || "None"), "calendar");
            },
            clearEndStyles : function () {
                ...
                ;
                if (!element) {
                    this.logWarn("No header-element in clearEndStyles()");
                    return;
                }
                var list = [this.baseStyle + "BothEnds", this.baseStyle + "LeftEnd",
                        this.baseStyle + "RightEnd"];
                for (var i=0; i<list.length; i++) {
                    element.classList.remove(list[i]);
                    header.classList.remove(list[i]);
                }
                delete this._currentEndStyle;
            }
        },
        ...
        draw : function () {
            var result = this.Super("draw", arguments);
            if (this._resetOnDraw) {
                delete this._resetOnDraw;
                this.reset();
                this.refreshEvents();
            }
            return result;
        ...
        getChildCanvas : function () {
            var canvas;
            if (this.canvasPool.length > 0) {
                // get a canvas from the pool
                canvas = this.canvasPool.shift();
                // and remove the pointy-end styles
                canvas.clearEndStyles();
            } else {
                // create a new canvas
                canvas = this.createAutoChild("canvas", {
                    height: this.canvasHeight,
                    calendarView: this.creator,
                    calendar: this.creator.creator
                });
        ...
        click : function () {
            // create a new allDay event at the click-location and edit it
            var dates = this.getMouseDates();
            if (!dates) return;

            // tweak the dates = we want a one-hour even, which is also flagged as allDay
            dates[0].setHours(new Date().getHours());
            dates[1].setHours(dates[0].getHours() + 1);

            var evt = isc.addProperties({}, { startDate: dates[0], endDate: dates[1] });
            evt[this.calendar.allDayField] = true;
            this.calendar.showEventEditor(evt, true);
            return false;
        },
        eventClick : function (canvas) {
            // edit an allDay or multi-day event
            var evt = isc.addProperties({}, canvas.event);
            this.creator.calendar.showEventEditor(evt);
            return false;
        },

        reset : function (start, end, fields) {
            // update the dates and hide current events

            // this.currentRecord is present if this canvas is displayed in one of
            // either 5 or 6 longEventsLayouts displayed as recordComponents in a
            // MonthView - if it's set, use the dates from the record - otherwise,
            // there's only one longEventsLayout in the view, so use the view dates
            var rec = this.currentRecord;
            start = start || (rec && rec.startDate) || this.calendarView.startDate;
            end = end || (rec && rec.endDate) || this.calendarView.endDate;
            this.logInfo(this.getID() + " -- reset(" + start.toShortDatetime() + ", " +
                end.toShortDatetime() + ")", "longEvents");
            this.updateFields(fields, start);
        },
        refreshEvents : function () {
            if (!this.isDrawn()) return;
            this.logInfo(this.getID() + " -- refreshEvents()", "longEvents");
            this.hideEvents();

            // refresh the local event list from the parent view's _longEventData
            var cal = this.calendar,
                view = this.calendarView,
                longEvents = this.calendarView.getLongEventData(),
                events = [],
                startTime = this.getStartDate().getTime(),
                endTime = this.getEndDate().getTime()
            ;
            for (var i=0; i<longEvents.length; i++) {
                var evt = longEvents[i];
                if (startTime >= evt[cal.endDateField].getTime()) continue;
                if (endTime <= evt[cal.startDateField].getTime()) continue;
                events.add(evt);
            }
        ...
        hideEvents : function () {
            // hide eventCanvases prior to update or removal
            if (!this.canvasList) return;
            var eventCount = this.canvasList.length;
            var revArr = this.canvasList.reverse();
            while (revArr.length > 0) {
                var canvas = revArr.shift();
                canvas.hide();
                canvas.clear();
                this.canvasPool.add(canvas);
                this.removeChild(canvas);
            }
            this.canvasList.length = 0;
            this.logInfo(this.getID() + " -- hideEvent: " + eventCount +
                " events hidden in layout", "longEvents");
        },

        updateFields : function (fields, startDate) {
            var view = this.calendarView,
                firstDateCol = view.getFirstDateColumn();

            // update the local field definitions from the ones on the grid
            if (!fields) fields = view.fields.duplicate();

            if (!startDate) {
                startDate = fields[firstDateCol].date && fields[firstDateCol].date.duplicate();
            }

            // make sure the startDate is logical
            startDate = isc.DateUtil.createLogicalDate(startDate);

            this.fieldWidths = view.getFieldWidths();

            this.fields = [];

            var isMonthView = this.calendarView.isMonthView();

            for (var i=0; i<fields.length; i++) {
                var newField = isc.addProperties({}, fields[i]);
                newField.width = this.fieldWidths[i];
                if (isMonthView)
                    newField.left = i == 0 ? 0 : this.fieldWidths.slice(0, i).sum();
                else
                    newField.left = i == 0 ? 0 : this.fieldWidths.slice(1, i).sum();
                if (i >= firstDateCol) {
                    newField.date = startDate.duplicate();
                    newField.startDate = isc.DateUtil.adjustDate(newField.date, "-0D");
                    newField.endDate = isc.DateUtil.adjustDate(newField.date, "+0D");
                    startDate = isc.DateUtil.adjustDate(startDate, "+1d");
                }
                this.fields.add(newField);
            }

            this.startDate = this.fields[0].startDate;
            this.endDate = this.fields[this.fields.length-1].endDate;

            this.logInfo(this.getID() + " -- updateFields: " +
                isc.echoFull(this.fields.getProperty("startDate")), "longEvents");
        },

        // sort the events with the longest at the top, so they'll appear at the same v-offset
        // in the longEventsLayouts in consecutive weeks in a monthView
        sortEvents : function () {
            this.eventList.setSort([
                { property:"eventLength", direction:"descending" }
            ]);
        },
        redrawEvents : function () {
            this.logInfo(this.getID() + " -- redrawEvents()", "longEvents");
            // sort the events
            this.sortEvents();

            if (!this.fields) this.updateFields();


            // redraw events according to their dates
            var eventCount = 0;

            var maxRight = this.getMaxEventRight();

            for (var i=0; i<this.eventList.length; i++) {
                var canvas = this.getChildCanvas();
                if (canvas) {
                    var evt = this.eventList[i],
                        left = this.getDateLeft(evt.startDate),
                        width = this.getEventWidth(evt)
                    ;
                    if (left + width > maxRight) width = maxRight - left;
                    canvas.resizeTo(width, this.canvasHeight);
                    canvas.setLeft(left);
                    canvas.setEvent(evt);
                    // add the canvas as a child without drawing it
                    this.addChild(canvas, null, false);
                    eventCount++;
                }
            }
            var height = this.layoutCanvases();
            if (eventCount > 0) this.setHeight(height);
            else this.setHeight(this.minHeight);
        },

        layoutCanvases : function () {
            var childCanvases = this.canvasList;
            var canvasHeight = this.canvasHeight;

            // track the "occupied" positions on the y-axis
            var occupiedPositions = [];

            var maxTop = 0;
            var maxRows = 0;

            // loop and position each child canvas
            for (var i = 0; i < childCanvases.length; i++) {
                var child = childCanvases[i];
                var topPosition = 0; // Start at the top (0px)
                var overlapFound = false;
                var rows = 0;

                // get the topmost position where the child canvas doesn't overlap with
                // any previous canvas
                while (true) {
                    overlapFound = false;

                    for (var j = 0; j < occupiedPositions.length; j++) {
                        var occupied = occupiedPositions[j];

                        // check for overlap in the x-axis
                        if (!(child.left + child.width <= occupied.left || child.left >= occupied.left + occupied.width)) {
                            // overlap in the x-axis, check for vertical overlap
                            if (topPosition < occupied.top + canvasHeight && topPosition + canvasHeight > occupied.top) {
                                overlapFound = true;
                                break;
                            }
                        }
                    ...
                    // if no overlap is found, place the canvas here
                    if (!overlapFound) {
                        break;
                    }

                    // otherwise, move the canvas down by the height
                    topPosition += canvasHeight;
                    rows++;
                }

                // set top on the child canvas, draw and show it
                child.setTop(topPosition + (rows * this.canvasSpacing));
                child.draw();
                child.show();

                maxTop = Math.max(maxTop, topPosition);

                // record the newly occupied position for future overlap checks
            ...
            this.logInfo(this.getID() + " -- layoutCanvases()", "longEvents");

            var top = maxTop + (rows * this.canvasSpacing);
            return top + this.canvasHeight + this.extraHeight;
        },

        getMaxEventRight : function () {
            if (this.fieldWidths) return this.fieldWidths.sum();
            return this.creator.getFieldWidths().sum();
        },

        getMouseColumn : function () {
            // return the start/end dates for the "column" that the mouse is over
            ...
            var widths = this.fieldWidths || [];
            for (var i = 0; i < widths.length; i++) {
                offset += widths[i]; // Add current field width to the cumulative width

                if (x < offset) {
                    // Return the index of the field containing the mouse offset
                    colNum = i;
                    break;
                }
            }
        ...
        getDateColumn : function (date) {
            // get the left offset for some date
            var colNum = -1;

            for (var i=0; i<this.fields.length; i++) {
                var field = this.fields[i];
                if (field.frozen || !field.endDate) continue;
                if (field.endDate.getTime() > date.getTime()) {
                    colNum = i;
                    break;
                }
            }
        ...
        getDateLeft : function (date) {
            // get the left offset for some date
            var colNum = this.getDateColumn(date);
            if (colNum == -1) {
                colNum = this.creator.frozenBody ? this.creator.frozenBody.fields.length : 1;
            }
            var frozenWidth = this.creator.frozenBody ? this.creator.frozenBody.getWidth() : 0;
            return this.fields[colNum].left + frozenWidth;
        },
        getDateRight : function (date) {
            // get the right offset for some date
            var colNum = this.getDateColumn(date);
            if (colNum == -1) return this.creator.getScrollWidth();
            var frozenWidth = this.creator.frozenBody ? this.creator.frozenBody.getWidth() : 0;
            var result = this.fields[colNum].left + this.fieldWidths[colNum] + frozenWidth - 1;
            return result;
        },
        getEventWidth : function (evt) {
            var left = this.getDateLeft(evt.startDate),
                right = this.getDateRight(new Date(evt.endDate.getTime())),
                width = right-left
            ;
            if (width < 0 || width > this.getScrollWidth()) width = this.calendarView.getScrollWidth();
            return width;
        },
        getMouseDates : function () {
            var colNum = this.getMouseColumn();
            if (colNum == null) return;

            var startDate = this.fields[colNum].startDate,
                endDate = this.fields[colNum].endDate
            ;
            return [startDate, endDate];
        }
    },

    // accepts the canvas being dragged so it's style can be replicated in the ghostCanvas
    showGhostCanvas : function (canvas, mouseStartDate, mouseEndDate) {
        if (!this.shouldAllowLongEvents()) return;

        this.ghostCanvas = canvas;
        this.ghostEvent = isc.addProperties({}, canvas && canvas.event);
        if (mouseStartDate) this.ghostEvent[this.calendar.startDateField] = mouseStartDate;
        if (mouseEndDate) this.ghostEvent[this.calendar.endDateField] = mouseEndDate;
        this.ghostMouseDates = [mouseStartDate, mouseEndDate];
        this.ghostLastMouseDates = [mouseStartDate, mouseEndDate];

        if (this.longEventsLayouts) {
            this.longEventsLayouts.callMethod("showGhostCanvas", canvas);
        } else if (this.longEventsLayout) {
            this.longEventsLayout.showGhostCanvas(canvas);
        }
    },
    updateGhostCanvas : function (mouseStartDate, mouseEndDate, reason) {
        if (!this.shouldAllowLongEvents()) return;

        var cal = this.calendar;

        var lDate = isc.DateUtil.createLogicalDate(mouseStartDate);
        var dates = this.getCurrentGhostRange(this.ghostEvent.anchorDate, lDate);

        // if the mouse-dates are the same as last time, just bail
        var startDelta = Math.round(isc.DateUtil.getPeriodLength(mouseStartDate,this.ghostLastMouseDates[0], "d"));
        if (startDelta == 0) return;

        this.logWarn("In updateGhostCanvas (reason: " + (reason || "none") + ":\n" + dates[0] + " \n " + dates[1]);

        var sign = (mouseStartDate > this.ghostLastMouseDates[0]) ? "+" : "-";

        this.ghostLastMouseDates = [mouseStartDate, mouseEndDate];

        var startDate = mouseStartDate,
            endDate = mouseEndDate
        ;


        if (this.inGhostDragCreate) {
            // when creating a new event, just affect the start or end date, depending on the
            // delta from the last mouse-date to the current one
            if (sign == "-")
                startDate = this.ghostEvent[cal.startDateField].duplicate();
            else
                endDate = this.ghostEvent[cal.endDateField].duplicate();
        } else {
            // otherwise, we're dragging an existing event - move both dates
        var adjuster = sign + Math.abs(startDelta) + "d";
            startDate = isc.DateUtil.adjustDate(this.ghostEvent[cal.startDateField], adjuster);
            endDate = isc.DateUtil.adjustDate(this.ghostEvent[cal.endDateField], adjuster);
        }

        // update the view-level ghostEvent, referenced by the separate longEventLayouts
        this.ghostEvent[cal.startDateField] = startDate;
        this.ghostEvent[cal.endDateField] = endDate;

        //this.logWarn("ghostEvent is: " + isc.echoFull(this.ghostEvent));

        if (this.longEventsLayouts) {
            // MonthView - has multiple layouts, one per row of day-cells
            this.longEventsLayouts.callMethod("updateGhostCanvas", startDate, endDate, "updateGhostCanvas");
        } else if (this.longEventsLayout) {
            // day/week views - just one layout
            this.longEventsLayout.updateGhostCanvas(startDate, endDate, "updateGhostCanvas");
        }

    },
    getCurrentGhostRange : function (date1, date2) {
        // Ensure we work with date objects
        var d1 = new Date(date1);
        var d2 = new Date(date2);

        // Get the start and end dates in the correct order
        var startDate = d1 < d2 ? d1 : d2;
        var endDate = d1 < d2 ? d2 : d1;
    ...
    // hides the current ghostCanvas from all longEventlayouts
    hideGhostCanvas : function () {
        if (!this.shouldAllowLongEvents()) return;

        if (this.longEventsLayouts) {
            this.longEventsLayouts.callMethod("hideGhostCanvas");
        } else if (this.longEventsLayout) {
            this.longEventsLayout.hideGhostCanvas();
        }

        if (this.inGhostDragMove) delete this.inGhostDragMove;
        if (this.inGhostDragCreate) delete this.inGhostDragCreate;

        //this.logWarn("dropped at " + this.ghostEvent.startDate + "<br>" + this.ghostEvent.endDate);
    },

    startGhostDragCreate : function (startDate, endDate) {
        if (!this.shouldAllowLongEvents()) return;
        var cal = this.calendar;
        this.ghostEvent = { name: "New Event", anchorDate: isc.DateUtil.getLogicalDateOnly(startDate) };
        this.ghostEvent[cal.startDateField] = startDate;
        this.ghostEvent[cal.endDateField] = endDate || startDate;
        this.inGhostDragCreate = true;
        this.showGhostCanvas(null, startDate, endDate);
        this.observeGhostEvents();
    },

    // called by an eventCanvas in a longEventsLayout, which passes itself
    startGhostDragMove : function (canvas) {
        if (!this.shouldAllowLongEvents()) return;
        var cal = this.calendar;
        this.ghostEvent = { name: "New Event" };
        var evt = canvas && canvas.event;
        var start = this.ghostEvent[cal.startDateField] = evt[cal.startDateField].duplicate();
        var end = this.ghostEvent[cal.endDateField] = evt[cal.endDateField].duplicate();
        this.ghostEvent.anchorDate = isc.DateUtil.getLogicalDateOnly(start);
        this.inGhostDragMove = true;
        this.showGhostCanvas(canvas, start, end);
        this.observeGhostEvents();
    },

    observeGhostEvents : function () {
        var _this = this;
        this._ghostKeyPressEventId = isc.Page.setEvent("keyPress", function (keyName) {
            _this.ghostDragKeyPress(keyName);
        });
        this._ghostMouseUpEventId = isc.Page.setEvent("mouseUp", function () {
            _this.ghostDragMouseUp();
        });
    },
    ignoreGhostEvents : function () {
        if (this._ghostKeyPressEventId != null) {
            isc.Page.clearEvent("keyPress", this._ghostKeyPressEventId);
            delete this._ghostKeyPressEventId;
        }
        if (this._ghostMouseUpEventId != null) {
            isc.Page.clearEvent("mouseUp", this._ghostMouseUpEventId);
            delete this._ghostMouseUpEventId;
        }
    },

    ghostDragKeyPress : function (keyName) {
        var key = keyName || isc.EH.getKeyEventCharacterValue ();
        if (key == "Escape" && (this.inGhostDragCreate || this.inGhostDragMove)) {
            this.logWarn("in ghostDragKeyPress - key is 'Escape' - calling cancelGhostDrag()")
            this.cancelGhostDrag();
        }
    },

    ghostDragMouseUp : function () {
        if (this.inGhostDragCreate) this.finishGhostDragCreate();
        if (this.inGhostDragMove) this.finishGhostDragMove();
        this.ignoreGhostEvents();
    },

    finishGhostDragCreate : function () {
        // delete the drag-create flag
        delete this.inGhostDragCreate;

        var cal = this.calendar;

        var startDate = this.ghostEvent[cal.startDateField],
            endDate = this.ghostEvent[cal.endDateField]
        ;

        this.ignoreGhostEvents();

        var newEvent = cal.createEventObject(null, startDate, endDate, null, null, "New Event");
        cal.showEventDialog(newEvent, true);
    },

    finishGhostDragMove : function () {
        // delete the drag-move flag
        delete this.inGhostDragMove;
        this.hideGhostCanvas();

        this.calendar.updateCalendarEvent(this.ghostCanvas.event, this.ghostEvent);
    },

    cancelGhostDrag : function () {
        // delete the drag-create and -move flags
        delete this.inGhostDragCreate;
        delete this.inGhostDragMove;

        this.ignoreGhostEvents();

        // and hide the ghostEvent
        this.hideGhostCanvas();
    },
    ...
    initWidget : function () {
        ...
        if (this.shouldAllowLongEvents() && (this.viewName == "day" || this.viewName == "week")) {
            this.longEventsLayout = this.createAutoChild("longEventsLayout", {
                calendarView: this,
                calendar: this.calendar,
                styleName: this.calendar.longEventLayoutStyleName,

                extraHeight: cal.longEventLayoutSpace,
                canvasHeight: cal.longEventHeight,
                canvasSpacing: cal.longEventSpace
            });
            this.gridComponents = ["header", this.longEventsLayout, "body"];
        }
    ...
    setLongEventData : function (events) {
        this._longEventData = events;
    },
    getLongEventData : function () {
        // get the local array of events
        if (!this._longEventData) this._longEventData = [];
        return this._longEventData;
    },
    ...
    setChosenDate : function (newDate) {
        ...
        // set the range dates
        if (this.rangeUnit) {
            this.periodStartDate = isc.DateUtil.getStartOf(this.chosenDate, this.rangeUnit, false, fDOW);
            this.startDate = this.periodStartDate.duplicate();
            this.periodEndDate = isc.DateUtil.getEndOf(this.chosenDate, this.rangeUnit, false, fDOW);
            this.endDate = this.periodEndDate.duplicate();
            ...
            if (this.isWeekView()) {
                ...
                // map the startDate/endDate to the week-boundaries - "periodStart/EndDate"
                // record the actual start and end of the month
                this.startDate = isc.DateUtil.getStartOf(this.chosenDate, "M", false, fDOW);
                this.startDate = isc.DateUtil.getStartOf(this.startDate, "W", false, fDOW)
                this.endDate = isc.DateUtil.getEndOf(isc.DateUtil.adjustDate(this.endDate, "-1MS"), "W", false, fDOW)
            }
    ...
    shouldAllowLongEvents : function () {
        if (this.allowLongEvents != null) return this.allowLongEvents;
        return this.calendar.allowLongEvents;
    },
...
// cell hovers
    ...
    asyncGetPrintHTML : function (printProperties, callback) {
        ...
        // if there are lanes, these might expand if any laneField-values were clipped -
        // deal with that here by measuring and applying the individual LaneField-values for
        // each Lane - then, call setLanes(), which will redraw with the overflowed heights
        // and place event canavses appropriately.  At the end of this method, we'll call
        // setLanes() again with the original clipped Lane-heights
        var lanes = this.calendar.lanes;
        if (lanes) {
            this.__prePrintLanes = [];
            for (var i=0; i<lanes.length; i++) {
                var lane = lanes[i];

                this.__prePrintLanes.add(isc.addProperties({}, lane));

                for (var j=0; j<this.calendar.laneFields.length; j++) {
                    var field = this.calendar.laneFields[j];
                    var props = {};
                    props.minWidth = field.minWidth;
                    props.width = field.width

                    var content = lane[field.name];
                    var styleName = this.getLaneFieldStyleName(field, lane);
                    var oldHeight = lanes[i].height;
                    var newHeight = isc.Canvas.measureContent(content, styleName, true, true, props);

                    //this.logWarn("oldHeight: " + oldHeight + " ... newHeight: " + newHeight);

                    if (newHeight > lane.height) lane.height = newHeight;
                }
            }
            this.calendar.setLanes(lanes);
        }
        ...
        var bodyVOffset = this.creator.dateLabel.getVisibleHeight();

        output.append("<div style='position:relative;top:" + bodyVOffset + "px;'>");
        ...
        if (this.__prePrintLanes) {
            this.calendar.setLanes(this.__prePrintLanes);
            delete this.__prePrintLanes;
        }
    ...
    eventDragCanvasDefaults: {
        ...
        dragRepositionMove : function () {
            ...
            allowDrop = cal.eventRepositionMove(event, newEvent, view);
        ...
        dragRepositionStop : function () {
            ...
            var continueUpdate = cal.eventRepositionStop(event, newEvent, otherFields, view);
            ...
            if (continueUpdate != false) {
                if (!isIndicator) {
                    ...
                    cal.updateCalendarEvent(event, newEvent, otherFields, "mouse");
                } else {
        ...
        dragResizeStop : function () {
            ...
            var continueUpdate = cal.eventResizeStop(event, newEvent, null, view);
            ...
            if (continueUpdate != false) {
                ...
                cal.updateCalendarEvent(event, newEvent, null, "mouse");
            }
    ...
    sizeEventCanvas : function (canvas, forceRedraw, calledFrom) {
        ...
        if (isTimeline) {
            this.logInfo("Sizing Timeline-event '" + event.name +"'" , "calendar");
            if (!lane) return;
        ...
        this.logDebug("New size for event '" + event.name + "': [" +
            [eTop,eLeft,eWidth,eHeight].join(", ") + "]", "calendar");
    ...
    */
    ...
    adjustDimensionsForOverlap : function (canvas, left, top, width, height) {
        ...
        if (props && props.totalSlots > 0) {
            ...
            if (isTimeline) {
                ...
                // override the slotSize if fixedEventHeight is set
                if (cal.fixedEventHeight) slotSize = cal.fixedEventHeight;

                // height is initially one slot
                height = slotSize;

                if (cal.fixedEventHeight == null) {
                    // only stretch events to fill multiple slots if fixedEventHeight isn't set
                    if (props.slotCount) {
            ...
            if (isTimeline) {
                // events with no overlap fill lane-height, unless fixedEventHeight is set
                if (cal.fixedEventHeight) height = cal.fixedEventHeight;
            } else {
                // leave an eventDragGap to the right of right-aligned events to allow
                // drag-creation of overlapping events
                width -= cal.eventDragGap || 1;
    ...
    refreshLongEvents : function () {
        if (this.longEventsLayout) {
            this.longEventsLayout.reset();
            this.longEventsLayout.refreshEvents();
        }
    },
    ...
    clearEvents : function (start, destroy) {
        ...
        // needed?
        this.clearLongEvents();
    },

    clearLongEvents : function () {
        if (this.longEventsLayout) this.longEventsLayout.reset();
    },
    ...
    drawZones : function () {
        // zones are doc'd as only being supported in Timelines
        if (!this.isTimelineView()) return;
        ...
        for (var i=0; i<rangeZones.length; i++) {
            var zone = rangeZones[i],
                ...
                // if zone.lane is set, size and position to that lane - otherwise,
                // use the sum of the lane-heights, even if that's less than the body height
                height = zone.lane ? this.getLaneHeight(zone.lane) : this.data.getProperty("height").sum(),
                top = zone.lane ? this.getRowTop(this.getLaneIndex(zone.lane)) : 0
            ;
            this.body.addChild(canvas, null, false)
            canvas.renderEvent(top, left, right-left, height, true);
    ...
    refreshEvents : function (caller) {
        ...
        // bail if the grid hasn't been drawn yet
        if (!this.body || !this.body.isDrawn()) return;

        // no data yet - kick off a fetch and bail
        if (!cal.hasData()) {
            // never fetch during the initial draw of the outer Calendar
            if (!cal._calendarDrawing) {
                this._refreshData();
            }
            //}
            return;
        ...
        while (--eventsLen >= 0) {
            ...
            if (!isc.isA.String(event)) {
                ...
                if (eStart.getHours() == 0 && eEnd.getHours() == 0) {
                    eEnd.setTime(eEnd.getTime() - 1);
                }

                var sameDay = isc.DateUtil.compareLogicalDates(eStart, eEnd) == 0;
                ...
                if ((eStart && eStart.getTime() >= startMillis && eStart.getTime() < endMillis) ||
                    ...
                    ((!sameDay || !this.verticalEvents) && eStart.getTime() <= startMillis && eEnd.getTime() >= endMillis)
                   )
                {
                    ...
                    var sameDay = isc.DateUtil.compareLogicalDates(eStart, eEnd) == 0;
                    if (!this.isTimelineView() && !sameDay || event[cal.allDayField]) {
                        this._longEventMap[event.__key] = event;
        ...
        this.setLongEventData(longEvents);
        this.tagDataForOverlap();

        // if using fixedEventHeight, need to redraw after data-overlapping
        // so the rows redraw at the right height for the contained events
        if (this.isTimelineView() && cal.fixedEventHeight) {
            this.redraw();
        }
    ...
    _refreshData : function (fromDraw) {
        if (this.__refreshDataRunning) return;
        this.__refreshDataRunning = true;
        var cal = this.calendar;
        var ds = cal.getDataSource();
        //isc.logWarn("nextOrPrev:" + cal.data.willFetchData(cal.getNewCriteria()));
        if (ds) {
            if (isc.ResultSet && isc.isA.ResultSet(cal.data)) {
                if (cal.hasData() && cal.shouldIncludeRangeCriteria(this) && !this.isTimelineView() && !this.isMonthView()) {
                    var rangeCrit = cal.getRangeCriteria(this);
                    if (rangeCrit && !isc.isAn.emptyObject(rangeCrit)) {
                        var lastCrit = cal._lastRangeCriteria;

                        if (lastCrit) {
                            var lastEndDate = lastCrit.criteria[0].value,
                                lastStartDate = lastCrit.criteria[1].value,
                                rangeEndDate = rangeCrit.criteria[0].value,
                                rangeStartDate = rangeCrit.criteria[1].value
                            ;
                            // if the new range is entirely within the previously-fetched range, just
                            // call refreshEvents() to update the UI from from the already-loaded
                            // range - otherwise, call filterData() below to re-fetch for the new range
                            if (isc.DateUtil.dateInRange(rangeStartDate, lastStartDate, lastEndDate) &&
                                isc.DateUtil.dateInRange(rangeEndDate, lastStartDate, lastEndDate))
                            {
                                isc.logInfo("New range-crit fully-contained - not fetching:\n\n" +
                                    isc.echoFull(rangeCrit), "calendar");
                                delete this.__refreshDataRunning;
                                this.refreshEvents();
                                return;
                            }
                        }
            ...
            // call fetchData() on the Calendar to refetch, or refreshEvents() on this view
            if (!cal.hasData() || (isc.isAn.Array(cal.data) )) {
                if (fromDraw && (cal._pendingFetchAfterDraw || cal.getCurrentViewName() != this.viewName)) {
                    delete cal._pendingFetchAfterDraw;

                    var callback = cal._pendingFetchCallback;
                    var requestProps = cal._pendingFetchRequestProperties;
                    delete cal._pendingFetchCallback;
                    delete cal._pendingFetchRequestProperties;

                    cal.fetchData(null, callback, requestProps);
                } else if (!fromDraw || (cal.autoFetchData && cal._pendingFetchAfterDraw)) {
                    delete cal._pendingFetchAfterDraw;
                    // only fetch if not being called from view.draw(), or if auto-fetching from cal.draw()
                    var callback = cal._pendingFetchCallback;
                    var requestProps = cal._pendingFetchRequestProperties;
                    delete cal._pendingFetchCallback;
                    delete cal._pendingFetchRequestProperties;
                ...
                cal.filterData(null, callback, requestProps);
            }
        } else {
    ...
    getEditDialogPosition : function (event) {
        var cal = this.calendar,
            ...
            rect
        ;
        if (cal.allowLongEvents && cal.isLongEvent(event)) {
            // show the eventDialog beneath the startDate for the long-event
            rect = [this.body.getColumnLeft(colNum), 0];
        } else {
            // otherwise, show the eventDialog in the startDate's grid-cell
...
isc.DaySchedule.addProperties({
    ...
    dateLabelCompactFormat: "ddd, MMM dd, YYYY",
    getDateLabelText : function (startDate, endDate) {
        if (this.isWeekView()) {
            return"<b>" + isc.DateUtil.getFormattedDateRangeString(startDate, endDate) + "</b>";
        }
        // on mobile, show a 3-char dayName rather than the full name
        return "<b>" + isc.DateUtil.format(startDate,
            isc.Browser.isMobile ? this.dateLabelCompactFormat : this.dateLabelFormat) + "</b>";
    },
    ...
    _getLabelFieldProperties : function () {
        return {
                autoFitWidth: this.calendar.autoFitLabelColumn,
    ...
    updateLongEventDates : function () {
        if (!this.isDrawn() || !this.longEventsLayout) return;

        // reset the layout to hide its children and resize it, and update its range-dates and
        // fields from this view
        this.longEventsLayout.reset();
        // and refresh/redraw event-children
        this.longEventsLayout.refreshEvents();
    },
    ...
    getCellDate : function (rowNum, colNum) {
        ...
        var fieldName = this.isDayView() ? "day1" : this.body.fields[colNum].name;
    ...
    draw : function (a, b, c, d) {
        ...
        if (this._refreshEventsOnDraw) {
            ...
            // run _refreshData() passing fromDraw:true, which may fetch or call refreshEvents()
            this.delayCall("_refreshData", [true], 100);
        }
    ...
    // return the fieldName for the field with the passed date - better than using colNum
    ...
    cellMouseDown : function (record, rowNum, colNum) {
        // can't drag-create events, just bail - always applies on mobile, where cellClick()
        // will show the new event window instead of a drag completing
        if (!this.calendar.canDragCreateEvents) return true;
        if (this.isLabelCol(colNum) || this.cellDisabled(rowNum, colNum)) return true;

        if (this.floatingScrollbars) {
            if (this.body.getOffsetX() > this.body.getScrollWidth() - this.scrollbarSize) {
                return;
            }
        }
    ...
    cellClick : function (record, rowNum, colNum) {
        // ignore cellClick() if canDragCreateEvents is true - we'll respond to cellMouseUp()
        // instead
        if (this.calendar.canDragCreateEvents) return;

        var cal = this.calendar,
            startDate = cal.getCellDate(rowNum, colNum, this),
            endDate = cal.getCellDate(rowNum+1, colNum, this)
        ;

        // bail if there's no cell-date
        if (!startDate) return;

        // bail if the cell-date is disabled (eg, disableWeekends is true and its a Sunday)
        if (this.calendar.shouldDisableDate(startDate)) return;
...
isc.MonthSchedule.addProperties({
    ...
    initWidget : function () {
        ...
        // prepare longEventsLayouts
        if (!this.shouldAllowLongEvents()) this.showRecordComponents = false;
        else {
            // other views have a single longEventsLayout - months have one per week
    ...
    updateRowDates : function () {
        var logicalDate = this.logicalDate.duplicate(),
            logicalTime = isc.DateUtil.createLogicalTime(0, 0, 0),
            nDate = logicalDate.duplicate()
        ;

        var dayNames = isc.DateUtil.getShortDayNames();
        var weekendDays = this.calendar.getWeekendDays();
        for (var i=0; i<this.data.length-2; i+=2) {
            var newRow = this.getHeaderRowObject(nDate);
            isc.addProperties(this.data[i], newRow);
            newRow = this.getEventRowObject(nDate);
            isc.addProperties(this.data[i+1], newRow);
            this.incrementDate(nDate, 7);
            //this.logWarn("ROW " + i + " UPDATE: " + isc.echoFull(rec));
        }
        //this.refreshLongEvents();
    },
    ...
    refreshLongEvents : function () {
        if (!this.longEventsLayouts || !this.shouldAllowLongEvents()) return;
        this.updateLongEventDates();

        for (var i=0; i<this.longEventsLayouts.length; i++) {
            var layout = this.longEventsLayouts[i];
            if (layout) {
                layout.reset();
                layout.refreshEvents();
            }
        }

    },

    // for now disable long-events in the monthView - some issues need to be fixed in this view
    ...
    // monthSchedule resized
    resized : function () {
        if (!this._dayArray || !this.body || !this.body.isDrawn()) return;
    ...
    getHeaderRowObject : function (theDate) {
        ...
        obj.startDate = isc.DateUtil.adjustDate(nDate.duplicate(), "-0D");
        for (var i=0; i<7; i++) {
        ...
        this.incrementDate(nDate, -1);
        obj.endDate = isc.DateUtil.adjustDate(nDate.duplicate(), "+0D");
    ...
    getDayArray : function () {
        ...
        // use the view's range- start/endDates, not logical dates, when getting events
        eventArr = this._eventData;

        /*
        cal._getEventsInRange(this.startDate, this.endDate, this);

        var normalEvents = [];
        for (var i=0; i<eventArr.length; i++) {
            if (!cal.isLongEvent(eventArr[i])) normalEvents.add(eventArr[i]);
        }
        eventArr = normalEvents;
        */
        ...
        if (this.body) this.cacheRowHeights(dayArr);
    ...
    refreshEvents : function () {
        ...
        // if no data yet, kick off a fetch and bail
        if (!cal.hasData()) {
            // fetch and return on the initial draw if there's a ds and autoFetchData is set
            if (this.shouldRefreshEventsOnDraw()) {
                this._refreshData();
                return;
            }
            // don't return otherwise - means the code below will still render the cells,
            // just without events, for the moment
        }

        var eventArr = cal._getEventsInRange(this.startDate, this.endDate, this);
        this._eventData = [];
        this._longEventData = [];
        for (var i=0; i<eventArr.length; i++) {
            if (this.shouldAllowLongEvents() && cal.isLongEvent(eventArr[i])) {
                this._longEventData.add(eventArr[i]);
            } else {
                this._eventData.add(eventArr[i]);
            }
        }
        ...
        // if long-events are not allowed, call showDataRows()
        //if (!this.shouldAllowLongEvents())
            this.showDataRows();
    ...
    getCellStyle : function (record, rowNum, colNum) {
        var cal = this.calendar,
            ...
            cellDisabled = cellDate ? cal.shouldDisableDate(cellDate) : false,
            isSelected = isc.DateUtil.compareLogicalDates(cellDate, this.chosenDate) == 0
        ;
    ...
    // monthView cellMouseDown
    cellMouseDown : function (record, rowNum, colNum) {
        var cal = this.calendar;

        // can't drag-create events, just bail - always applies on mobile, where cellClick()
        // will show the new event window instead of a drag completing
        if (!cal.canCreateEvents || !cal.canDragCreateEvents) return true;
        // disabled cell, can't start a drag
        if (this.cellDisabled(rowNum, colNum)) return true;
        ...
        // if backgroundMouseDown is implemented, run it and return if it returns false
        var startDate = this.getCellDate(this.body.getEventRow(), this.body.getEventColumn()),
            endDate = this.getCellEndDate(this.body.getEventRow(), this.body.getEventColumn());

        this.startGhostDragCreate(startDate, endDate);
    ...
    // monthView cellOver
    cellOver : function (record, rowNum, colNum) {
        if (this.inGhostDragCreate || this.inGhostDragMove) {
            var startDate = this.getCellDate(rowNum, colNum),
                endDate = this.getCellEndDate(rowNum, colNum)
            ;
            this.updateGhostCanvas(startDate, endDate, "cellOver");
        }
    },

    // monthView cellMouseUp
    cellMouseUp : function (record, rowNum, colNum) {
        var cal = this.calendar;
        // can't drag-create events, just bail - always applies on mobile, where cellClick()
        // will show the new event window instead of a drag completing
        if (this.inGhostDragCreate) {
            this.finishGhostDragCreate();
            return isc.EH.STOP_BUBBLING;
        }
        if (this.inGhostDragMove) {
            this.finishGhostDragMove();
            return isc.EH.STOP_BUBBLING;
        }
    },
    ...
    cellClick : function (record, rowNum, colNum) {
        if (this.inGhostDragCreate) return false;
        var cal = this.calendar,
            year, month,
            fieldIndex = this.fields.get(colNum)._dayIndex,
            currDate = record["date" + fieldIndex],
        ...
        // with clicks in a cell, we usually need to set the global selected chosenDate, to
        // update the month in this view and mark day/week views for a refresh, as
        // required - we'll do that later, if the handlers don't return false

        if (this.rowIsHeader(rowNum)) { // header clicked
            doDefault = cal.dayHeaderClick(clickDate, evtArr, cal, rowNum, colNum);
            if (doDefault) {
                // call setChosenDate(), so that changing tab below refreshes the day tab
                cal.setChosenDate(clickDate);

                // change to the day tab - the calendar-level chosenDate has already been set
                cal.selectTab("day");
            }
            ...
            if (!this.cellDisabled(rowNum, colNum)) {
                ...
                if (doDefault && cal.canCreateEvents) {
                    ...
                    if (isOtherDay) {
                        // if clicking otherDay-bodies navigates, setChosenDate() now
                        if (cal.otherDayClickNavigation) cal.setChosenDate(clickDate);
                        // if not allowing editing on otherDays, bail
                        if (!cal.canCreateOtherDayEvents) return;
                    } else {
                        cal.setChosenDate(clickDate);
                    }

                    var newEvent = cal.createEventObject(null, startDate, endDate, null, null, "New Event");
                    if (cal.allowLongEvents) newEvent[cal.allDayField] = true;
                    cal.delayCall("showEventDialog", [newEvent, true]);
                }
            }
    ...
    draw : function ( ) {
        ...
        // once drawn, cache the row-heights right away
        this.cacheRowHeights();
        ...
        if (this._fireViewChangedOnDraw) {
            delete this._fireViewChangedOnDraw;
            cal.currentViewChanged(this.viewName);
        }

        // force a redraw now that we have the cached heights
        this.markForRedraw();
    },

    // get [rowNum, colNum] for the passed date
    getDateCell : function (date) {
        for (var i=0; i<this.data.length; i++) {
            for (var j=1; j<=this.fields.length; j++) {
                if (isc.DateUtil.compareLogicalDates(date, this.data[i]["date" + j]) == 0) {
                    return [i, j-1]
                }
            }
        }
    ...
    // MonthView
    getEditDialogPosition : function (event) {
        var cell = event.startDate ? this.getDateCell(event.startDate) :
                [this.getEventRow(), this.getEventColumn()]
        ;

        var rect = this.body.getCellPageRect(cell[0], cell[1]);
        return {
            left: Math.max(rect[0], this.body.getPageLeft()),
            top: Math.max(rect[1], this.body.getPageTop())
        };
...
isc.TimelineView.addProperties({
    ...
    recordDrop : function (dropRecords, targetRecord, index, sourceWidget) {
        ...
        // handle an internal record drag-reorder
        if (cal.canReorderLanes && sourceWidget == this) {
            // at this point, the drop has happened and this.data is in the order we want it to
            // be in - we need to update the top-level lanes array because updateLanes() is
            // called while refreshing data and will revert the dropped lane to its old position
            var newLanes = [];
            // re-sort calendar.lanes to match the record-order, post-drop
            for (var i=0; i<cal.lanes.length; i++) {
                newLanes[i] = cal.lanes.find("name", this.data[i].name);
            }
            // apply the re-ordered lanes array
            cal.lanes = newLanes;
        }
    ...
    tagDataForOverlap : function (data, lane) {
        ...
        var laneSlotCounts = {};
        for (var i=0; i<olRanges.length; i++) {
            var range = olRanges[i];
            if (!laneSlotCounts[range.lane]) laneSlotCounts[range.lane] = 0;
            if (range.slotList.length > laneSlotCounts[range.lane]) {
                // cache the max number of rows needed in each lane
                laneSlotCounts[range.lane] = range.slotList.length;
            }
        }
        this.laneSlotCounts = laneSlotCounts;
        if (cal.fixedEventHeight) {
            // update the heights on the records to reflect the events they span
            for (var key in laneSlotCounts) {
                var record = this.data.find("name", key);
                if (record) {
                    record.height = laneSlotCounts[key] * cal.fixedEventHeight;
                    record.height += (laneSlotCounts[key] + 1) * cal.laneEventPadding;
                    record.height = Math.max(record.height, cal.minLaneHeight);
                }
            }
        }
    },
    ...
    buildSnapGapList : function (reason) {
        ...
        // if the timeline is using a large granularity, where length in MS is not guaranteed
        // (such as week/month/year), set the periodLength here and the loop below will use it
        // to calculate the proper MS-value for each unit of granularity - for example, May
        // and June never have the same length, and February is longer in leap years
        var periodLength = null;
        if (isc.DateUtil.compareTimeUnits("day", this.timelineGranularity) == 1) {
            periodLength = this.timelineGranularity;
        }

        while (currTime < endTime) {
            if (periodLength) {
                // if there's a periodLength, the granularity is a week or more and the
                // snapTime may differ from one unit to the next
                snapTime = isc.DateUtil.getUnitDurationForDate(currTime, periodLength);
            }
            ...
            // if there's a periodLength, it's a large granularity - don't take 1ms off
            // the end-time
            if (!periodLength) {
                nextTime--;
            }
    ...
    _rebuild : function (refreshData) {
        ...
        this.setShowHover(cal.showViewHovers);

        this._rebuildFields();

        if (!this._skipSetLanes) {
            var lanes = this.lanes || cal.lanes || [];
        ...
        if (!cal._doneInitialDraw) {
            return;
        }

        // don't fetch on initial draw unless there's no DS or autoFetchData is true
        if (!cal.hasData() || (ds && isc.isAn.Array(cal.data))) {
    ...
    setLanes : function (lanes, skipRefreshEvents) {
        ...
        // need to redraw the body here, before trying to align events to the new lanes
        this.redraw();
    ...
    setTimelineRange : function (start, end)
    {
        ...
        //if (!cal.hasData()) refreshData = false;
        if (cal._calendarDrawing) refreshData = false;
    ...
    calcFields : function () {
        ...
        while (sDate.getTime() <= eDateMillis) {
            ...
            if (showDate) {
                // show the date in the displayTimezone in the field-header
                var title = this.getInnerFieldTitle(headerLevel, spanIndex, thisDate);
    ...
    getHeaderSpans : function (startDate, endDate, headerLevels, levelIndex, fields) {
        ...
        if (levelIndex > 0) {
            if (isc.DateUtil.compareTimeUnits(unit, lastUnit) == -1) {
    ...
    getInnerFieldTitle : function (headerLevel, spanIndex, startDate, endDate) {
        ...
        if (!result) {
            ...
            if (granularity == "year") {
                ...
                var time = isc.DateUtil.getLogicalTimeOnly(startDate);
                result = isc.Time.toShortTime(time, this.calendar.timeFormatter);
            }
            if (isc.isA.Function(headerLevel.titleFormatter)) {
    ...
    slideRange : function (slideRight) {
        ...
        // refresh data after changing range
        this._refreshData();
...
// default hover auto-sizing settings - these will propagate to child views and eventCanvases,
// which set them to null, unless devs provide them in defaults or properties
hoverAutoFitWidth: true,
hoverAutoFitMaxWidth: 300,
...
// The name of the field which will determine the +link{Calendar.lanes, lane} in which a given
...
//> @attr calendar.allDayField (String : "allDay" : IR)
// The name of the field on a +link{CalendarEvent} which indicates an event should fill the
// whole day.  Such events are displayed in a separate
// +link{calendarView.longEventsLayout, long-events layout}, which appears beneath the headers
// in the +link{weekView, week} and +link{dayView, day} views and does not scroll with the
// main body.  In the +link{monthView, month} view, these layouts appear beneath the headers
// for each week, and very long events might wrap from one week to the next.
//
// @group calendarEvent
// @visibility internal
// @see CalendarEvent
...
// when this is true and minLaneWidth is set, labelColumnWidth acts as a minimum for auto-fit
autoFitLabelColumn: false,
...
shouldDisableDate : function (date, view, isEndDate) {
    ...
    view = view || this.getSelectedView();
    if (view.isTimelineView()) {
        // if showing a Timeline with a granularity > "day", never disable the cell
        if (isc.DateUtil.compareTimeUnits("day", this.timelineGranularity) == 1) {
            return false;
        }
    }
...
//> @attr calendar.otherDayClickNavigation (Boolean : true : IRW)
// When +link{calendar.showOtherDays} is true, this attribute determines whether the month-view
// should change month when cells representing days in the previous or following month are
// clicked.
//
// @visibility calendar
//<
otherDayClickNavigation: true,

//> @attr calendar.canCreateOtherDayEvents (Boolean : false : IRW)
// When +link{calendar.showOtherDays} is true, determines whether clicking in a Month-view
// body-cell that represents a date outside the selected month will open the
// +link{calendar.eventEditor, event-editor window} at the cell's location.
// <p>
// Note that, when +link{calendar.otherDayClickNavigation} is also true, the
// Month-view first switches to the month of the clicked date before displaying the
// event-editor.  This causes the editor to open in a different cell than the one clicked and
// may be unexpected to users.
// @visibility calendar
//<
canCreateOtherDayEvents: false,
...
getVisibleStartDate : function (view) {
    ...
    if ((view.body && view.bodies.length > 1)) return view.getCellDate(0, 0);
...
//> @attr calendar.dataFetchMode (FetchMode : "basic" : IRW)
...
// @value "auto" events are fetched according to the range in the visible view
// @visibility internal
//<
...
//> @attr calendar.eventAllDayFieldTitle (HTMLString : "All Day" : IR)
// The title for the +link{allDayField} in the detailed +link{calendar.eventEditor, editor}.
// @group i18nMessages
// @visibility internal
//<
eventAllDayFieldTitle: "All Day",
...
//> @attr calendar.monthButtonIconSrc (SCImgURL : "Chevron_Left" : IR)
// The icon to show next to the month-name in the +link{calendar.monthButton, month button},
// used for showing and hiding the +link{calendar.monthView, month view} on Handsets.
// @visibility external
//<
monthButtonIconSrc: "Chevron_Left",
...
//> @attr calendar.backButtonIconSrc (SCImgURL : "Chevron_Left" : IR)
// The icon to show in the +link{calendar.monthButton, month-button} on Handsets when the
// +link{calendar.monthView, month view} is the current visible view.
// @visibility external
//<
backButtonIconSrc: "Chevron_Left",
...
eventEditorDefaults : {
    padding: 4,
    numCols: 9,
    colWidths: [ 80, 40, 80, 80, 80, 80, "*", "*", "*" ],
    showInlineErrors: false,
    // fill the containing layout - fixed width is no good on mobile in portrait
    width: "100%",
    titleWidth: 80,
...
//> @attr calendar.controlBarIconSize (Integer : 16 : IR)
// The size of the various icons displayed in the +link{calendar.controlsBar, controlsBar}
// of this Calendar.
// @visibility calendar
//<
controlBarIconSize: 16,

//> @attr calendar.controlBarIconBaseStyle (CSSStyleName : "icon" : IR)
// A CSS style to apply to icons in the +link{controlsBar}.  This is a base style supporting
// suffixes for states, specifically "Over", "Down" and "Disabled", which are applied when
// +link{class:ImgButton} settings like +link{imgButton.showRollOverIcon} are applied to the
// icons.
// @visibility calendar
//<
controlBarIconBaseStyle: "icon",

//> @attr calendar.controlsBarHeight (Integer : 30 : IR)
// Default height of the +link{controlsBar} shown above the main Calendar grid.  When multiple
// views are available and tabs are visible, this value is modified to ensure that the content
// of the <code>controlsBar</code> is vertically-aligned with the text in the tabs.
// @visibility calendar
//<
controlsBarHeight: 34,
...
initWidget : function () {
    //if (this.autoFetchData) delete this.autoFetchData;
...
updateMonthButton : function () {
    if (this.getCurrentViewName() == "month") {
        this.monthButton.setProperties({
            icon: this.backButtonIconSrc,
        ...
        this.monthButton.setProperties({
            icon: this.monthButtonIconSrc,
            iconAlign: this.monthButtonIconAlign,
            title: this.monthButtonTitle.evalDynamicString(this, { monthName: month })
        });
        this.monthButton.redraw();
    }
},

pageOrientationChanged : function (orientation) {
    orientation = orientation || isc.Page.getOrientation();
    // in "landscape", show the weekView if it's available - for "portrait", show the dayView -
    // setCurrentViewName() eventually calls selectTab(), which will lazily create the view as
    // needed
    if (orientation == "landscape" && this.showWeekView) this.setCurrentViewName("week");
    if (orientation == "portrait" && this.showDayView) this.setCurrentViewName("day");
},
...
//this.logWarn("observed ARRAY data");
    if (this.hasData()) {
...
hasData : function () {
    ...
    if (ds && this.data) {
        if (isc.ResultSet && isc.isA.ResultSet(this.data)) {
            if (this.data.lengthIsKnown()) return true;
            return false;
        } else if (isc.isAn.Array(this.data)) {
            if (this.data.length > 0) return true;
            return false;
        }
    }

    // don't do this - breaks updates for non-databound calendars
    //return false;
...
dataChanged : function () {
    ...
    //this.logWarn("*** " + this.getSelectedView().viewName + " dataChanged(" + isc.echoFull(arguments) + ")");

    //this.refreshSelectedView();
    var view = this.getSelectedView();
    if (view) view.refreshEvents();
    return;
},
...
getEventHeaderHTML : function (event, view) {
    ...
    if (name) {
        if (this.shouldEscapeFieldHTML(this.nameField) || (canvas && canvas.escapeHTML)) {
            name = name.asHTML();
        }
    }
    return eTitle + (name || "");
...
getEventBodyHTML : function (event, view) {
    ...
    var canvas = view.getCurrentEventCanvas(event);
    if (defaultValue) {
        if (this.shouldEscapeFieldHTML(this.descriptionField) || (canvas && canvas.escapeHTML)) {
...
getEventStartDate : function (event, view) {
    var date = event && event[this.startDateField];
    if (!date) return null;
    if (!isc.isA.Date(date)) {
        date = Date.parseStandardDate(date);
        if (!isc.isA.Date(date)) return null;
        event[this.startDateField] = date.duplicate()
    }
...
getEventEndDate : function (event, view) {
    ...
    if (!isc.isA.Date(date)) {
        date = Date.parseStandardDate(date);
        if (isc.isA.Date(date)) event[this.endDateField] = date.duplicate()
    }
    if (duration != null) {
...
addLaneEvent : function (laneName, startDate, endDate, name, description, otherFields) {
    ...
    this.addCalendarEvent(newEvent, otherFields, "none");
},
...
addEvent : function (startDate, endDate, name, description, otherFields, laneName, ignoreDataChanged) {
    ...
    this.addCalendarEvent(newEvent, otherFields, "none", ignoreDataChanged);
},
...
// <p>
// To deal with errors during saving, see +link{calendar.eventSaveError}.
...
// @visibility calendar
//<
addCalendarEvent : function (event, customValues, scenarioCode, ignoreDataChanged) {
    if (!event) return;
    // We explicitly update the UI in this method, so no need to react to 'dataChanged' on the
    // data object
    if (ignoreDataChanged == null && !isc.isAn.Array(this.data)) ignoreDataChanged = true;
    ...
    if (this.dataSource) {
        var _this = this;
        var requestProps = {componentId: this.ID, willHandleError: true, scenarioCode: scenarioCode};
        ...
        isc.DataSource.get(this.dataSource).addData(event, function (dsResponse, data, dsRequest) {
            _this.processSaveResponse(dsResponse, dsRequest);
        }, requestProps);
        return;
    } else {
        // don't set the one-time flag to ignore data changed - we need a refresh
        this._ignoreDataChanged = false;
        this.data.add(event);
        this.processSaveResponse({status:0, data:[event]}, {operationType: "add",
...
updateEvent : function (event, startDate, endDate, name, description, otherFields, ignoreDataChanged, laneName, sublaneName) {
    ...
    this.updateCalendarEvent(event, newEvent, otherFields, "none", ignoreDataChanged);
},

//> @type CalendarSaveScenario
// Indicates how a given Calendar Save was initiated - values are <i>dialog</i>, <i>editor</i>
// and <i>mouse</i>.
//
// @value "dialog"  the user saved via the +link{calendar.eventDialog}
// @value "editor"  the user saved via the +link{calendar.eventEditor}
// @value "mouse"   the user moved or resized an +link{class:EventCanvas, event} with the mouse
// @visibility external
//<

//> @method calendar.eventSaveError()
// Notification method fired when an attempt to save +link{class:CalendarEvent, an event}
// following edits or drag-movements results in an error from the server.  May be overridden
// to handle specific errors and provide feedback to the user or push a server-provided record
// to client caches with a call to +link{dataSource.updateCaches, updateCaches()}, for example.
// <p>
// The <code>scenarioCode</code> parameter may be used to determine
// +link{type:CalendarSaveScenario, how the save was initiated}.
// <p>
// Return <i>false</i> from this method to cancel builtin behavior, which is as follows:
// <p>
// If a save happened because an event-canvas was moved or sized with the mouse, and any kind
// of error occurs, the event is
// restored to it's pre-save position - if this method specifically returns <i>true</i>, the
// errors are also sent to +link{group:errorHandling,central error handling},
// which will show them to the user in a simple error dialog.
// <p>
// If a save happened because an event was edited in the +link{eventDialog} or
// +link{eventEditor} windows and a validation error occurs, the window is re-opened for
// editing with the validation errors visible.  If any other kind of error occurs, the window
// will not be re-opened and, if this method specifically returns <i>true</i>, the error will
// be sent to +link{group:errorHandling,central error handling} which will show a
// simple error dialog to the end user.
// <p>
// In all cases, if the server returned an updated record despite the save failing (eg, perhaps
// the server enforces optimistic locking and the underlying record has been changed by another
// user), it is assumed to be a latest-version of the record and is applied instead of the
// original record during those builtin UI processes.  So, the +link{eventEditor} and
// +link{eventDialog} will show any changes from the latest-record.  If the
// save was caused by dragging an event and the event's +link{startDateField, start} or
// +link{endDateField, end} dates are different in the latest-record,
// the EventCanvas will be positioned using the latest values, instead of returning to
// it's pre-save position on-screen.
//
// @param dsResponse (DSResponse) DSResponse from the save-attempt
// @param dsRequest  (DSRequest) initiating DSRequest Properties
// @param scenarioCode  (CalendarSaveScenario) scenario in which the save occurred
// @return (Boolean) false to cancel builtin behavior - null to allow builtin behaviors, true
//                   to also allow central event-handling for unhandled errors
// @visibility external
//<
eventSaveError : function (dsResponse, dsRequest, scenarioCode, ignoreDataChanged) {
    return true;
},

//> @method calendar.updateCalendarEvent()
// Save an event to this Calendar's +link{calendar.dataSource, dataSource) or
// +link{calendar.data, data array}.  To deal with errors while saving, see
// +link{calendar.eventSaveError}
//
...
// @visibility calendar
//<
updateCalendarEvent : function (event, newEvent, otherFields, scenarioCode, ignoreDataChanged) {
    // see comment above dataChanged about _ignoreDataChanged
    if (ignoreDataChanged) this._ignoreDataChanged = true;
    otherFields = otherFields || {};
    ...
    if (this.dataSource) {
        ...
        var requestProps = { oldValues: oldEventRecord, componentId: this.ID, willHandleError: true,
                operationType: "update", scenarioCode: scenarioCode };
        if (this.updateOperation) requestProps.operationId = this.updateOperation;

        var _this = this;
        ds.updateData(updatedRecord, function (resp, req, ds) {
            _this.processSaveResponse(resp, req, oldEventRecord);
        }, requestProps);
        return;
    } else {
        var oldEvent = isc.addProperties({}, event);
        isc.addProperties(event, newEvent, otherFields);
        this.processSaveResponse({status:0, data:[event]},
            {operationType:"update", scenarioCode: scenarioCode}, oldEvent);
    }
},

processSaveResponse : function (dsResponse, dsRequest, oldEvent) {
    var newEvent = isc.isAn.Array(dsResponse.data) ? dsResponse.data[0] : dsResponse.data,
        opType = (dsRequest && dsRequest.operationType) || (dsResponse && dsResponse.operationType),
        isUpdate = opType == "update",
        isAdd = opType == "add",
        fromDialog = this._fromEventDialog,
        fromEditor = this._fromEventEditor,
        allDayChange = this._allDayChange
    ;
    ...
    delete this._allDayChange;

    if (dsResponse && dsResponse.status < 0) {
        // fire eventSaveError() - if it returns false (Cancel), skip the internal stuff that
        // we would do otherwise
        var handleSaveResult = this.eventSaveError(dsResponse, dsRequest, dsRequest.scenarioCode);
        if (handleSaveResult == false) {
            return;
        } else {
    ...
    // if the data is an array, replace the existing record by __key if it was an update and
    // fire dataChanged() to refresh whatever view is selected - then bail
    if (isc.isAn.Array(this.data)) {
        if (isUpdate) {
            // overwrite the original event-record
            var oldIndex = oldEvent ? this.data.findIndex("__key", oldEvent.__key) : -1;
            if (oldIndex != -1) isc.addProperties(this.data[oldIndex], newEvent);
        } else {
            // get the event-key for a new event
            this.getEventKey(newEvent);
        }

        // for fixed event-heights, redraw the Timeline to have the rows resize to fit events
        if (this.fixedEventHeight && view.isTimelineView()) view.redraw();

        // for an add operation, addCalendarEvent() already added the record to the array
        this.dataChanged();
        return;
    }

    if (allDayChange || !isUpdate) {
        if (!view.isMonthView()) view.addEvent(newEvent);

        if (view.isMonthView()) {
            // in the monthView, just invalidate the cache to force a full reload because new
            // longEvents always need a full redraw
            this.invalidateCache();
        } else {
            // use view._refreshEvents() - this.invalidateCache() works but is visibly slower
            //this.invalidateCache();
            view._refreshEvents();
        }
        return;
    ...
    if (this._shouldRefreshTimeline(startDate, endDate)) {
        if (!this.timelineViewSelected()) this.timelineView._needsRefresh = true;
            ...
            if (this.fixedEventHeight) view.redraw();
            if (isUpdate) {
...
// check for a DS field with escapeHTML set
shouldEscapeFieldHTML : function (field) {
    var ds = this.getDataSource(),
        field = ds && ds.getField(field)
    ;
    return field && field.escapeHTML;
},
...
_getEventHoverHTML : function (event, eventCanvas, view) {
    ...
    if (name) {
        ...
        if (this.shouldEscapeFieldHTML(this.nameField) || (eventCanvas && eventCanvas.escapeHTML))
            name = name.asHTML();
        sb.append("<br><br>", name);
    }
    if (description) {
        // support eventCanvas.escapeHTML for event-descriptions
        if (this.shouldEscapeFieldHTML(this.descriptionField) || (eventCanvas && eventCanvas.escapeHTML))
...
_getCellHoverHTML : function (view, record, rowNum, colNum) {
    ...
    if (field.isLaneField) {
        ...
        defaultValue = record && record[field.name];
        ...
        if (view.isTimelineView()) {
            // for timelines, if the time-span of the cell is not a day, we want to show the
            // time and/or date-range
            var sDate = view.getCellDate(rowNum, colNum - (view.frozenFields ? view.frozenFields.length : 0)),
                eDate = view.getCellEndDate(rowNum, colNum - (view.frozenFields ? view.frozenFields.length : 0))
            ;
            defaultValue = this.formatDateRange(sDate, eDate);
        } else {
...
// format a date-range for display in the UI - deals with allDay and multi-day ranges
formatDateRange: function (sDate, eDate) {
    return this._getEventDialogTitle(sDate, eDate, { startDate: sDate, endDate: eDate });
},
...
_getHeaderHoverHTML : function (view, headerLevel, button, startDate, endDate) {
    ...
    if (headerLevel && headerLevel.hoverHTML) {
...
// @visibility external
//<
getEventCanvasStyle : function (event, view, canvas) {
    view = view || this.getSelectedView();
    if (canvas && canvas.isLongEvent && this.longEventStyleName && this.shouldAllowLongEvents()) {
        // for longEvents, use the longEventStyleName
        return this.longEventStyleName;
    }
...
_createEventCanvasControls : function (canvas, focusControl) {
    ...
    if (this.canRemoveEvents != false) {
        ...
        controls.closeButton = this.createAutoChild("eventCanvasCloseButton", {
            width: this.eventCanvasCloseButtonSize,
            height: this.eventCanvasCloseButtonSize,
            imageWidth: this.eventCanvasCloseIconSize,
            imageHeight: this.eventCanvasCloseIconSize
        });
...
//> @attr calendar.eventCanvasCloseButtonSize (Integer : 16 : IR)
// The size of the +link{calendar.eventCanvasCloseButton, close-button} that snaps to the
// top-right of an event canvas on rollover and shows allows an event to be removed from a
// +link{class:CalendarView}.
// @visibility external
//<
eventCanvasCloseButtonSize: 16,

//> @attr calendar.eventCanvasCloseIconSize (Integer : 13 : IR)
// The size of the icon in the +link{calendar.eventCanvasCloseButton, close-button} floated
// over events on rollover.
// @visibility external
//<
eventCanvasCloseIconSize: 13,
...
getEventCanvasCloseButton : function (canvas) {
    var buttonSize = this.creator.eventCanvasCloseButtonSize,
        iconSize = this.creator.eventCanvasCloseIconSize
    ;
...
_getEventStyleName : function (evt) {
    // support the deprecated eventWindowStyle attribute
    return evt[this.eventWindowStyleField] || evt[this.eventStyleNameField];
},
...
columnLayoutDefaults: {
    ...
    drop : function () {
        ...
        this.calendar.updateCalendarEvent(canvas.event, newEvent, null, "mouse");
...
_getEventCanvas : function (event, view) {
    ...
    if (props.hoverAutoFitWidth == null) {
        props.hoverAutoFitWidth = this.hoverAutoFitWidth;
    }
    if (props.hoverAutoFitMaxWidth == null) {
        props.hoverAutoFitMaxWidth = this.hoverAutoFitMaxWidth;
    }
...
_getEventsInRange : function (start, end, view, visibleLanesOnly) {
        ...
        for (var i = 0; i < dataLength; i++) {
            var curr = this.data.get(i),
                ...
                eventEnd = this.getEventEndDate(curr)
            ;
            ...
            // October 2024 - we support longEvents now, so include any event that overlaps
            // the range, rather than only events which start and end in the range
            if (this.shouldAllowLongEvents()) {
                if (eventEnd.getTime() > start.getTime() && eventStart.getTime() < end.getTime()) {
                ...
                // start-time needs >= to include events that start exactly on the view-start
...
// Gets the criteria to use when the calendar date ranges shift.  This would be called, for
// example, when the next button is clicked and new events may need to be fetched. Override
// this function to add any custom criteria to the default criteria constructed by the calendar.
//
...
//> @attr calendar.includeRangeCriteria (Boolean : true : IR)
// When set to true, the default, the fetches issued by navigating around in the various views
// are limited to the accessible date-range in the current view - as you change the current
// date-range, a fetch is only issued if the new range is not entirely within the previous range.
// <p>
// For example, navigating in the +link{monthView} will fetch all events in the range of 5 or
// 6 weeks that cover that month.  If you then change to the +link{dayView, day} or
// +link{weekView, week} views, no fetches will be issued until you navigate outside of the
// 5 or 6 weeks initially fetched by the Month view.
// @visibility external
//<
includeRangeCriteria: true,
...
_filter : function (type, criteria, callback, requestProperties, doneSaving) {
    ...
    if (!view || !view.isDrawn()) {
        if (!this._pendingFetchAfterDraw && view && view.isTimelineView()) {
            this.logDebug("_filter called on Timeline: _fetchAfterDraw set to true:" + this.getStackTrace());
        }
        this._pendingFetchAfterDraw = true;

        // cache the params for the delayed call later
        this.initialCriteria = criteria;
        this._pendingFetchCallback = callback;
        this._pendingFetchRequestProperties = requestProperties;
        return;
    }
    if (view.isTimelineView()) {
        this.logDebug("_filter called on Timeline: " + this.getStackTrace());
    }

    // if passed criteria, assign it to initialCriteria - it's the base-criteria
    // that will be extended with range-criteria below
    if (criteria) this.initialCriteria = criteria;

    // base criteria is initialCriteria - dev provided
    criteria = isc.DS.copyCriteria(this.initialCriteria);
    ...
    if (this.shouldIncludeRangeCriteria(view)) {
        ...
        if (rangeCrit) {
            ...
            requestProperties.sortBy = [
                { property: this.startDateField, direction: "ascending" },
                { property: this.endDateField, direction: "ascending" }
            ];
        }
        criteria = isc.DS.compressNestedCriteria(criteria);
        // cache the last rangeCriteria that was fetched - if a view needs to refresh,
        // it will only fetch if it's range-dates are outside of the last rangeCriteria
        this._lastRangeCriteria = isc.DataSource.copyCriteria(rangeCrit);
    }
    return this.Super("_filter", arguments);
},

setCriteria : function (criteria) {
    // in this widget, setCriteria() applies explicit crit which is extended with
    // automatic date-range criteria - store the dev-provided crit in this.initialCriteria
    this.initialCriteria = criteria;
},

getCriteria : function () {
    var data = this.data;
    if (data && data.getCriteria) return data.getCriteria();
    return isc.DS.copyCriteria(this.initialCriteria);
},
...
dataArrived : function () {
    //this.logWarn("*** " + this.getSelectedView().viewName + " dataArrived(" + isc.echoFull(arguments) + ")");
    // if _observeDataArrived
    if (this._observeDataArrived) this.dataChanged();
    var view = this.getSelectedView();
    if (view && view.isTimelineView() || view.isMonthView())
        view.refreshEvents();
        //this.dataChanged();
    return true;
},

// override invalidateCache() to cause an immediate re-fetch for the current view's range
invalidateCache : function () {
    var result = this.Super("invalidateCache", arguments);

    // clear the previous rangeCriteria so refreshSelectedView() will cause a fetch, even
    // though its current range hasn't changed
    delete this._lastRangeCriteria;

    // refresh the selected view - this will re-fetch data limited to the date-range of the
    // selected view and mark the other views to refresh their data the next time they're
    // shown - if those views have a larger range, another fetch will occur at that time
    this.refreshSelectedView();

    return result;
},

// refreshData() needs to invalidate the cache in order to re-fetch with the same date-range
...
draw : function (a, b, c, d) {
    ...
    if (this._pendingFetchAfterDraw) {

        // respect autoFetchData on the initial draw
        if (this.autoFetchData || this._doneInitialDraw) {
            this.refreshData();
        }
    }
    ...
    this._doneInitialDraw = true;
},



// allow Google-like multi-day interface - not yet finished
allowLongEvents: false,

shouldAllowLongEvents : function () {
    return this.allowLongEvents;
},

//> @attr calendar.longEventLayoutStyleName  (CSSStyleName : "longEventLayout" : IRW)
// The base name for the CSS class applied to the layouts that show
// +link{calendarView.longEventsLayout, long-events} in the various
// +link{class:CalendarView, CalendarViews}.
//
// @group appearance
// @visibility internal
//<
longEventLayoutStyleName: "longEventLayout",

//> @attr calendar.longEventLayoutSpace (Integer : 10 : IRW)
// The amount of space available in +link{calendarView.longEventsLayout, long-event layouts}
// for drag-click and drag-creation of new events.  Represents the default height of long-event
// layouts that have no events, and additional height in layouts with events.
// @group appearance
// @visibility internal
//<
longEventLayoutSpace: 10,


//> @attr calendar.longEventDragOpacity  (Integer : 30 : IRW)
// The default +link{canvas.opacity, opacity} of +link{calendar.allDayField, long-events} while
// being dragged.
// @group appearance
// @visibility internal
//<
longEventDragOpacity: 30,

//> @attr calendar.longEventHeight (Integer : 20 : IRW)
// The default height of the canvases for +link{calendar.allDayField, long-events} which are
// displayed in the separate +link{calendarView.longEventsLayout}.
// @group appearance
// @visibility internal
//<
longEventHeight: 20,

//> @attr calendar.fixedEventHeight (Integer : null : IRW)
// When set, applies a fixed height to horizontal events in Timelines, rather than allowing
// them to share the whole height of their Lane.  Overlapping behavior is unchanged, but all
// events are of the same fixed height and are stacked in the Lane, which may overflow to
...
//> @attr calendar.minLaneHeight (Integer : 40 : IRW)
// The minimum height for Lanes in a Timeline.  When events have a
// +link{calendar.fixedEventHeight, fixed height}, Lanes will size just tall enough to
// accommodate their events but will not shrink below this minimum height.
// @group appearance
// @visibility external
//<
minLaneHeight: 40,

//> @attr calendar.longEventSpace (Integer : 1 : IRW)
// The vertical space between the canvases for +link{calendar.allDayField, long-events} when
// displayed in the separate +link{calendarView.longEventsLayout}.
// @group appearance
// @visibility internal
//<
...
//> @attr calendar.longEventStyleName  (CSSStyleName : "longEvent" : IRW)
// The base name for the CSS class applied to +link{calendar.eventCanvas, events} when they're
// rendered in a +link{calendarView.longEventsLayout, long-events layout} in a view.
// This style will have "Header" and "Body" appended to it, according to
// which part of the event window is being styled.  By default, only the Header.
//
// @group appearance
// @visibility internal
//<
longEventStyleName: "longEvent",

//> @method calendar.getLongEventHTML()
// Returns the text to be displayed in a +link{calendar.allDayField, long-event} displayed
// horizontally in the separate +link{calendarView.longEventsLayout, long-events layout} above
// vertical Calendar views.
// @param event (CalendarEvent) the event to get the description text for
// @param layout (Canvas) the longEventsLayout in which this event is displayed
// @param [view] (CalendarView) the view in which the event is being rendered
// @return (HTMLString) the HTML to display in the header of an event canvas
// @visibility internal
//<
getLongEventHTML : function (event, layout, view) {
    if (!event) return null;

    var result = "",
        start = this.getEventStartDate(event),
        end = this.getEventEndDate(event),
        allDay = !!event[this.allDayField],
        multiDay = isc.DateUtil.compareLogicalDates(start, end) != 0
    ;

    if (multiDay) {
        if (allDay) {
            // just dates
            result = start.toShortDate() + " - " + end.toShortDate();
        } else {
            // dates and times
            result = start.toShortDatetime() + " - " + end.toShortDatetime();
        }
    } else {
        // just allDay
        result = start.toShortDate();
    }

    if (allDay) result = " [" + this.eventAllDayFieldTitle + "] " + result;

    var name = event[this.nameField];
    ...
    // clip long text and show ellipsis
    return "<span style='overflow:hidden; text-overflow:ellipsis;'><nobr>" +
        (name ? name + " - " : "") + result + "</nobr></span>";
},

//> @method calendar.getLongEventHoverHTML()
// Returns the text to be displayed in a +link{canvas.showHover, hover} when the mouse is held
// over a given +link{class:EventCanvas, event} in the
// +link{calendarView.longEventsLayout, separate layout} where
// +link{calendar.isLongEvent(), long-events} are displayed.
// @param event (CalendarEvent) the event to get the description text for
// @param layout (Canvas) the longEventsLayout in which this event is displayed
// @param [view] (CalendarView) the view in which the event is being rendered
// @return (HTMLString) the HTML to display in the header of an event canvas
// @visibility internal
//<
getLongEventHoverHTML : function (event, layout, view) {
    return this.getLongEventHTML(event, layout, view);
},

//> @method calendar.getLongEventLayoutHoverHTML()
// Returns the text to be displayed in a +link{canvas.showHover, hover} when the mouse is held
// over the +link{calendarView.longEventsLayout, separate layout} where
// +link{calendar.isLongEvent(), long-events} are displayed.
// @param layout (Canvas) the longEventsLayout over which the mouse is hovered
// @param [view] (CalendarView) the view in which the event is being rendered
// @return (HTMLString) the HTML to display in the hover for this longEventsLayout
// @visibility internal
//<
getLongEventLayoutHoverHTML : function (layout, view) {
    var dates = layout.getMouseDates();
    if (!dates) return;
    return isc.DateUtil.format(dates[0], "dddd, dd MMMM yyyy");
},

//> @method calendar.isLongEvent()
// Returns true if the passed event is marked as lasting +link{calendar.allDayField, all day},
// or it +link{calendar.startDateField, starts} and +link{calendar.endDateField, ends} on
// different days in your current locale.  These types of events are displayed horizontally
// in a +link{calendarView.longEventsLayout, separate layout} placed above the date cells in
// CalendarViews.
// @param event (CalendarEvent) the event in question
// @param [multiDayOnly] (Boolean) when true, only matches events that span multiple days -
//          otherwise, matches those events or +link{calendar.allDayField, all-day events}.
// @return (Boolean) true if the event lasts all day or starts and ends on different days
// @visibility internal
//<
isLongEvent : function (event, multiDayOnly) {
    if (!multiDayOnly && event[this.allDayField]) return true;
    var start = this.getEventStartDate(event),
        end = this.getEventEndDate(event)
    ;
    if (isc.DateUtil.compareLogicalDates(start, end) != 0) return true;
    return false;
},
...
_getTabPane : function (viewName) {
    ...
    if (viewName == "day") {
        ...
        // set up the monthView with longEventsLayout autoChildren under each
        // row of day-"headers"
        var recordCompProps = {};
        if (this.shouldAllowLongEvents()) {
            recordCompProps = {
                showRecordComponents: true,
                recordComponentPosition: "above",
                //recordComponentPosition: "within",
                recordComponentPoolingMode: "recycle",
                extraHeight: this.longEventLayoutSpace,
                canvasHeight: this.longEventHeight,
                canvasSpacing: this.longEventSpace,
                createRecordComponent: function (record, colNum) {
                    if (record.isHeaderRow) {
                        //this.logWarn("record " + this.getRecordIndex(record) + " is a header");
                        return;
                    }
                    var widget = this.createAutoChild("longEventsLayout", {
                        calendarView: this,
                        calendar: this.calendar,
                        styleName: this.calendar.longEventLayoutStyleName
                    });
                    this.longEventsLayouts.add(widget);
                    return widget;
                },
                updateRecordComponent: function (record, colNum, component, recordChanged) {
                    if (record.isHeaderRow) return;
                    component._resetOnDraw = true;
                    return component;
                }
            }
        }
        this.monthView = this.createAutoChild("monthView",
            isc.addProperties({}, recordCompProps, props, {
...
_getTabs : function () {
    ...
    if (this.showDayView != false) {
        nTabs.add({title: this.dayViewTitle, pane: null, viewName: "day", name: "day" });
    }
    if (this.showWeekView != false) {
        nTabs.add({title: this.weekViewTitle, pane: null, viewName: "week", name: "week" });
    }
    if (this.showMonthView != false) {
        nTabs.add({title: this.monthViewTitle, pane: null, viewName: "month", name: "month" });
    }
    if (this.showTimelineView != false) {
        nTabs.add({title: this.timelineViewTitle, pane: null, viewName: "timeline", name: "timeline" });
    }
    return nTabs;
},
...
createChildren : function () {
    ...
    if (this.showControlsBar != false) {
        ...
        this.addEventButton = this.createAutoChild("addEventButton", {
            baseStyle: this.controlBarIconBaseStyle,
            width: this.controlBarIconSize,
            height: this.controlBarIconSize,
            click : function () {
                var cal = this.creator;
        ...
        if (this.minimalUI && this.showMonthButton != false && this.showMonthView != false) {
            ...
            // we need h-padding but not v-padding - just clear the v-padding here, rather
            // than adding dozens of lines of new CSS for a new stateful style, just for padding
            this.monthButton.setTopPadding(0);
            this.monthButton.setBottomPadding(0);
    ...
    // set up controlsBar with the calendar date-label and navigation controls
    var cbProps = { members: cbMems };
    // make the controlsBar the same height as the TabSet's tabBar if mainView is a TabSet,
    // so the date-label and navigation controls v-align with text in the tabButtons
    if (isc.isA.TabSet(this.mainView)) cbProps.height = this.mainView.tabBarThickness;
    else cbProps.height = this.controlsBarHeight;

    if (this.showControlsBar != false) {
        this.controlsBar = this.createAutoChild("controlsBar", cbProps );
    }
...
getEventDialog : function () {
    ...
    this.eventDialog = this.createAutoChild("eventDialog", {
            ...
            isc.DynamicForm.create({
                ...
                submit : function () {
                    ...
                    if (!isNewEvent) { // event window clicked, so update
                        cal.updateCalendarEvent(evt, newEvent, customValues, "dialog");
                    } else { // create new event
                        cal.addCalendarEvent(newEvent, customValues, "dialog");
                    }
                },
        ...
        setDate : function (startDate, endDate) {
            ...
            this.setTitle(cal._getEventDialogTitle(startDate, endDate, this.event));
            if (this.isNewEvent) {
        ...
        hide : function () {
            this.creator.getSelectedView().hideGhostCanvas();
    ...
    this.addChild(this.eventDialog);
    return this.eventDialog;
},

//> @attr calendar.eventEditorButtons (Array of Any : (see below) : IR)
// The list of buttons to include in the +link{calendar.eventEditor}.  Entries can be
// the names of the builtin buttons, +link{calendar.saveButton, saveButton},
// +link{calendar.removeButton, removeButton} and +link{calendar.cancelButton, cancelButton},
// or the names of custom +link{type:AutoChild, autoChildren} that have been
// defined on the Calendar instance, or widget instances that already exist.
// <p>
// The default is to show the builtin buttons.
// @visibility external
//<
eventEditorButtons: ["saveButton", "removeButton", "cancelButton"],
...
getEventEditorLayout : function () {
    ...
    this.eventEditor = this.createAutoChild("eventEditor", {
        useAllDataSourceFields: true,
        titleWidth: 80,
        // have this layout fill the Calendar, as it's now a child
        width: "100%", height: "100%",
        calendar: this,
        initWidget : function () {
        ...
        rebuildFieldList : function () {
            ...
            this._internalFields = [cal.nameField, cal.laneNameField, cal.sublaneNameField];
            ...
            fieldList.add({ name: cal.startDateField, title: cal.eventStartDateFieldTitle,
                type: "datetime", colSpan: 2, endRow: !cal.shouldAllowLongEvents(),
                showIf : function (item, value, form, values) {
                    // show datetime startDate if multi-day but not all-day
                    return form.getValue("multiDay") && !form.getValue("allDay");
                },
                changed : function (form, item, value) {
                    var layout = form.calendar.eventEditorLayout;
                    layout.setCurrentStart(value);
                }
            });
            fieldList.add({ name: "logicalStartDate", title: cal.eventEditorDateFieldTitle,
                type: "date", colSpan: 2, endRow: !cal.shouldAllowLongEvents(),
                showIf : function (item, value, form, values) {
                    // always show logical startDate for all-day events
                    return !form.getValue("multiDay") || form.getValue("allDay");
                },
                changed : function (form, item, value) {
                    var layout = form.calendar.eventEditorLayout;
                    var newD = layout.currentStart.duplicate();
                    newD.setFullYear(value.getFullYear());
                    newD.setMonth(value.getMonth());
                    newD.setDate(value.getDate());
                    layout.setCurrentStart(newD);
                }
            });

            // All-Day and and Multi-Day checkboxes
            fieldList.add({ name: cal.allDayField, title: cal.eventAllDayFieldTitle,
                    type: "boolean", colSpan: 1, endRow: false, startRow: false, showTitle: false,
                    showIf : function (item, value, form, values) {
                        // show the All Day checkbox for non-Timelines if longEvents are allowed
                        return cal.shouldAllowLongEvents() && !view.isTimelineView();
                    },
                    changed : function (form, item, value) {
                        var evt = isc.addProperties({},
                                form.parentElement.creator.event, form.getValues());
                        form.updateFieldsForLongEvent(evt)
                    }
            });
            fieldList.add({ name: "multiDay", title: cal.eventMultiDayFieldTitle,
                    type: "boolean", colSpan: 1, endRow: true, startRow: false, showTitle: false,
                    textColor: "orange",
                    showIf : function (item, value, form, values) {
                        // show the Multi-Day checkbox for non-Timelines if longEvents are allowed
                        return cal.shouldAllowLongEvents() && !view.isTimelineView();
                    },
                    changed : function (form, item, value) {
                        var evt = isc.addProperties({},
            ...
            // endDate fields - one is a datetime and the other logical
            fieldList.add({ name: cal.endDateField, title: cal.eventEndDateFieldTitle,
                type: "datetime", colSpan: "*", endRow: true,
                showIf : function (item, value, form, values) {
                    // don't show any end date if durations are used
                    if (form.creator.allowDurationEvents) return false;
                    // show datetime endDate if multi-day but not all-day
                    return form.getValue("multiDay") && !form.getValue("allDay");
                },
                changed : function (form, item, value) {
                    var layout = form.calendar.eventEditorLayout;
                    layout.setCurrentEnd(value);
                }
            });
            fieldList.add({ name: "logicalEndDate", title: cal.eventEndDateFieldTitle,
                type: "date", colSpan: 2, endRow: !cal.shouldAllowLongEvents(),
                showIf : function (item, value, form, values) {
                    // don't show any end date if durations are used
                    if (form.creator.allowDurationEvents) return false;
                    // show logical endDate if both multi-day and all-day
                    return form.getValue("multiDay") && form.getValue("allDay");
                },
                changed : function (form, item, value) {
                    var layout = form.calendar.eventEditorLayout;
                    var newD = layout.currentEnd.duplicate();
                    newD.setFullYear(value.getFullYear());
                    newD.setMonth(value.getMonth());
                    newD.setDate(value.getDate());
                    layout.setCurrentEnd(newD);
                }
            });
            ...
            if (editStyle == "date" || editStyle == "datetime") {
                ...
                fieldList.addList([
                    {name: "startHours", title: cal.eventStartDateFieldTitle, type: "integer",
                        ...
                        changed : function (form, item, value) {
                            var layout = form.calendar.eventEditorLayout;
                            var d = layout.currentStart.duplicate();
                            d.setHours(value);
                            layout.setCurrentStart(d);
                        }
                    },
                ...
                fieldList.addList([
                    {name: "endHours", type: "integer", width: "*", startRow: true,
                        ...
                        changed : function (form, item, value) {
                            var layout = form.calendar.eventEditorLayout;
                            var d = layout.currentEnd.duplicate();
                            d.setHours(value);
                            layout.setCurrentEnd(d);
                        }
                    },
        ...
        updateFieldsForLongEvent : function (event) {
            // - if multiday is true
            //   - if allDay is true
            //     - show the logicalStartDate and hide the startDate and logical time-fields
            //     - show logicalEndDate
            //   - if not
            //     - show the startDate endDate datetime fields
            //       - hide the logicalStart/EndDate and the time-fields
            // - if multiDay is false
            //   - hide the startDate, endDate and logicalEndDate fields
            //   - show the logicalStartDate and logical time-fields
            //   - if not
            //     - show the datetime startDate field
            //     - hide and hide to/from time-fields
            var cal = this.calendar,
                values = this.getValues()
            ;

            var timeFields = ["startHours", "startMinutes","endHours","endMinutes"];
            if (!cal.twentyFourHourTime) fields.addList(["startAMPM", "endAMPM"]);

            if (values.multiDay) {
                if (values.allDay) {
                    // !values.multiDay || !values.allDay
                    this.hideItem(cal.startDateField);
                    this.hideItem(cal.endDateField);
                    // values.multiDay && values.allDay
                    this.showItem("logicalStartDate");
                    this.showItem("logicalEndDate");
                } else {
                    this.showItem(cal.startDateField);
                    this.showItem(cal.endDateField);
                    this.hideItem("logicalStartDate");
                    this.hideItem("logicalEndDate");
                }
                // always hide the logical start-time items
                for (var i=0; i<timeFields.length; i++) {
                    this.hideItem(timeFields[i]);
                }
            } else {
                this.hideItem(cal.startDateField);
                this.hideItem(cal.endDateField);
                this.hideItem("logicalEndDate");
                this.showItem("logicalStartDate");
                // hide the logical start-time items
                for (var i=0; i<timeFields.length; i++) {
                    if (values.allDay) this.hideItem(timeFields[i]);
                    else this.showItem(timeFields[i]);
                }
            }

            this.parentElement.creator.setTitle(
                cal._getEventDialogTitle(event[cal.startDateField], event[cal.endDateField], event)
            );
...
//            } else if (editStyle == "time") {
                ...
                form.setValue("logicalEndDate", this.logicalEndDate);
...
//            }
        ...
        createButtonLayout : function () {
            ...
            this.buttonLayout = cal.createAutoChild("eventEditorButtonLayout");
            this.saveButton = cal.createAutoChild("saveButton",
                { title: cal.saveButtonTitle, calendar: this.calendar });
            this.removeButton = this.calendar.createAutoChild("removeButton",
                { title: cal.removeButtonTitle, calendar: this.calendar });
            this.cancelButton = this.calendar.createAutoChild("cancelButton",
                { title: cal.cancelButtonTitle, calendar: this.calendar });
            var buttons = cal.eventEditorButtons;
            var members = [];
            for (var i=0; i<buttons.length; i++) {
                var b = buttons[i];
                if (isc.isAn.Instance(b)) members.add(b);
                else if (isc.isA.String(b)) {
                    if (this[b]) members.add(this[b]);
                    else if (cal[b]) members.add(cal[b]);
                    else if (cal[b + "Defaults"]) {
                        // an autoChild - create it
                        this[b] = cal.createAutoChild(b, { calendar: this.calendar });
                        members.add(this[b]);
                    }
                }
            }
            this.buttonLayout.addMembers(members);
        ...
        setCurrentStart : function (value) {
            ...
            this.logicalStartDate = isc.DateUtil.createLogicalDate(value);
            this.logicalStartTime = isc.DateUtil.createLogicalTime(value);
            // update the UI
            form.setValue(cal.startDateField, this.currentStart);
            form.setValue("logicalStartDate", this.logicalStartDate);
            form.setValue("startHours", this.logicalStartTime.getHours());
            form.setValue("startMinutes", this.logicalStartTime.getMinutes());
            //form.setValue("startAMPM", null);

            // if "Multi-Day" is unchecked and the startDate changes, the endDate needs to
            // change as well
            if (!form.getValue("multiDay")) {
                var logicalEndTime = isc.DateUtil.createLogicalTime(this.currentEnd);
                var currentEnd = isc.DateUtil.combineLogicalDateAndTime(this.logicalStartDate, logicalEndTime);
                this.setCurrentEnd(currentEnd);
            }
        },
        setCurrentEnd : function (value) {
            var form = this.items[0];
            this.currentEnd = value;
            this.logicalEndDate = isc.DateUtil.createLogicalDate(value);
            this.logicalEndTime = isc.DateUtil.createLogicalTime(value);
            // update the UI
            form.setValue(cal.endDateField, this.currentEnd);
            form.setValue("logicalEndDate", this.logicalEndDate);
            form.setValue("endHours", this.logicalEndTime.getHours());
            form.setValue("endMinutes", this.logicalEndTime.getMinutes());
            //form.setValue("endAMPM", null);
        },
        ...
        setEvent : function (event) {
            ...
            var startDate = cal.getEventStartDate(event),
                endDate = cal.getEventEndDate(event)
            ;
            this.setDate(startDate, endDate, event);
            ...
            var allDayEvent = event[cal.allDayField];
            form.setValue(cal.allDayField, allDayEvent);
            // set the multiDay checkbox - isLongEvent() will return true for an allDay event,
            // unless you pass the second param, multiDayOnly: true
            form.setValue("multiDay", cal.isLongEvent(event, true));

            form.updateFieldsForLongEvent(event);
...
hideEventDialog : function () {
    ...
    this.getSelectedView().hideGhostCanvas();
},
...
addEventOrUpdateEventFields : function () {
    var cal = this,
        ...
        allDayChange = form.valueHasChanged(this.allDayField)
    ;
    ...
    if (this.shouldAllowLongEvents()) {
        newEvent[this.allDayField] = values[this.allDayField];
    }
...
*/
    ...
    newEvent[cal.startDateField] = values[this.startDateField];
    newEvent[cal.endDateField] = values[this.endDateField];
    ...
    if (allDayChange) cal._allDayChange = true;

    if (!isNewEvent) {
        cal.updateCalendarEvent(evt, newEvent, customValues, "editor");
    } else {
        cal.addCalendarEvent(newEvent, customValues, "editor", false);
    }
    return true;
},
...
_showEventDialog : function (event, isNewEvent) {
    ...
    // ensure the dialog won't render off the right of the Calendar
    var thisWidth = this.getPageLeft() + this.getWidth();
    var dWidth = dialog.getVisibleWidth();
    var dLeft = offset.left;
    if (dLeft + dWidth >= thisWidth) {
        dLeft = thisWidth - (dWidth + 2);
    }

    dialog._lastRect = [dLeft, dTop];
    dialog.placeNear(dialog._lastRect[0], dialog._lastRect[1]);
    dialog.show();
    dialog.bringToFront();
...
_getEventDialogTitle : function (startDate, endDate, event) {
    ...
    var isMultiDay = event && this.isLongEvent(event, true);
    var isAllDay = event && !!event[this.allDayField];

    if (isMultiDay) {
        // multi-day - need a format like Jan 1 - Feb 2 - with times if not isAllDay
        result = days[startDate.getDay()] + ", " + months[startDate.getMonth()] + " " +
                    startDate.getDate() + (!isAllDay ? ", " + sTime : "") + " - " +
                 days[endDate.getDay()] + ", " + months[endDate.getMonth()] + " " +
                    endDate.getDate() + (!isAllDay ? ", " + eTime : "")
        ;
        return result;
    } else if (isAllDay) {
        // all-day - need a format like Jan 1 [All Day]
        result = days[startDate.getDay()] + ", " + months[startDate.getMonth()] + " " +
                    startDate.getDate() + " [" + this.eventAllDayFieldTitle + "]"
        ;
        return result;
    }

    // single day, not allDay - need a format like Jan 1, 10:00 - 12:00
    result = days[startDate.getDay()] + ", " + months[startDate.getMonth()] + " " +
                startDate.getDate() + " - " + sTime + " - " + eTime
    ;
    return result;
},
...
getCellDate : function (rowNum, colNum, view) {
    ...
    if (view.isDayView() || view.isWeekView() || view.isTimelineView()) {
        ...
        if (rec && rec["date" + dIndex] != null) {
            ...
            retDate = isc.DateUtil.createDatetime(retDate, null, null, null, 0, 0, 0, 0);
        }
    } else {
        return;
    }
...
// <P>
// Note that, when +link{calendar.otherDayClickNavigation} is true, the calendar will first
// navigate the Month view to the selected month, before showing the Event Editor dialog at
// the proper location.
// <p>
...
_eventCanvasClick : function (canvas, defaultOnly) {
    var event = canvas.event,
        ...
        // always run the click-handler - an override may cancel it and thus this flow,
        // so we'll return (clickResult != false) from this method as a fallback
        clickResult = this.eventClick(event, view.viewName),
        doDefault = (defaultOnly ? true : clickResult) != false
    ;
    ...
    if (doDefault) {
        ...
        if (!view.isTimelineView()) {
            /* whatever this code was meant to do, it's incomplete - comment it */
            /*
    ...
    // bubble the return-value from the eventClick() handler, which may be customized
...
_eventCanvasKeyPress : function (canvas, key) {
    ...
    if (key) {
        if (key == "Enter") {
            ...
            if (cache.canEdit) result = this._eventCanvasClick(canvas, true);
...
selectTab : function (tabnum) {
    if (!this.mainView || !this.mainView.isA("TabSet")) return false;

    // remember the current tab-number so the button can be refreshed below
    var currentTabNum = this.mainView.getSelectedTabNumber();

    // support being passed any supported tab identifier value (not just tabnum)
    var tab = this.mainView.getTabObject(tabnum);

    if (tab) {
        this.mainView.selectTab(tabnum);
        if (this.mainView.tabBar) {
            // focus the button for the new tab, new tabButton if it's there
            var button =  this.mainView.tabBar.getButton(tabnum);
            if (button) button.focus();
            // update the button for the previous tab, if it's there, in case it was focused
            button = this.mainView.tabBar.getButton(currentTabNum);
            if (button) button.stateChanged();
        }
        return true;
    }
    return false;
...
// @return (Boolean) return false to cancel the default drag start behavior
// @visibility external
//<
eventRepositionStart : function (event, view) {
    return true;
},
...
isc.EventCanvas.addProperties({
    ...
    getPartHandle : function (part) {
        part = part || "canvas";
        if (part == "header" && this.getShowHeader()) return this.headerLabel.getHandle();
        if (part == "body" && this.getShowBody()) return this.bodyLabel.getHandle();
    ...
    init : function () {
        ...
        this.autoFit = this.calendar && this.calendar.showColumnLayouts;
        if (this.autoFit) {
            this.height = 1;
    ...
    initWidget : function () {
        ...
        if (this.verticalResize == null) {
            this.verticalResize = view.verticalEvents;
        }


        this.resizeFrom = [];

        this.hoverDelay = cal.hoverDelay + 1;

        if (this.useStaticControls == null) {
            this.useStaticControls = (cal.useEventCanvasRolloverControls == false);
        }
        if (this.canFocus == null) {
            this.setCanFocus(cal.canSelectEvents);
        }
    ...
    leadingTrailingIconDefaults: {
        ...
        dragRepositionStop : function () {
            ...
            cal.updateCalendarEvent(eventObj, newEvent, null, "mouse");
    ...
    _updateValueCache : function (event, styleName, headerStyle, bodyStyle) {
        ...
        if (this.event) {
            ...
            if (this.isLongEvent) cache.canDragResize = false;
            ...
            cache.eventStyleName = cal.getEventCanvasStyle(event, view, this);
    ...
    updateRolloverControls : function (mouseOver) {
        ...
        if (this.closeButton) {
            if (!cache.showCloseButton) this.closeButton.hide();
                ...
                // update the close-icon's CSS color to match the text color - doesn't affect
                // real icons of course, only SVG or font-icons
                var handle = this.closeButton.getStyleHandle();
                if (handle) {
                    handle["color"] = this.getShowHeader() ? this.headerTextColor : this.bodyTextColor;
                }
    ...
    setEventStyle : function (styleName, headerStyle, bodyStyle) {
        ...
        if (this.needsColorUpdateOnSetEvent || this.styleName != styleName) {
            ...
            if (this.getShowHeader()) {
                this.headerLabel.setStyleName(this._headerStyle);
                var style = isc.Element._deriveStyleProperties(this._headerStyle, {color:"color"});
                this.headerTextColor = style["color"];
            }
    ...
    updateCanvasColors : function () {
        ...
        if (e.textColor || e.backgroundColor || e.borderColor) {
            ...
            if (e.textColor || e.backgroundColor) {
                ...
                if (bodyHandle) {
                    ...
                    if (e.textColor) {
                        bodyHandle["color"] = e.textColor;
                        this.bodyTextColor = bodyHandle["color"];
                    }
                }
            }
            domAccessed = true;
        ...
        if (e.headerTextColor || e.headerBackgroundColor || e.headerBorderColor) {
            ...
            if (headerHandle) {
                ...
                if (e.headerTextColor) {
                    headerHandle["color"] = e.headerTextColor;
                    this.headerTextColor = headerHandle["color"];
                }
...
// generating HTML
    ...
    checkStyle : function () {
        var styleName = this.calendar.getEventCanvasStyle(this.event, this.calendarView, this);
...
// internal stuff - mouse handler
    click : function () {
        ...
        var result = this.calendar._eventCanvasClick(this);
        return result;
    },
...
//> @attr calendarView.longEventsLayout (MultiAutoChild Canvas : null : IR)
// A separate container that provides an intuitive way for users to view and interact with
// multi-day events, or events marked as lasting +link{calendar.allDayField, all day}.  If
// you've seen the calendars provided by Google, Apple or Microsoft, you'll already be familiar
// with the approach.
// <p>
// In the +link{dayView, day} and +link{weekView, week} views, this component appears beneath
// the headers and does not scroll with the main body.  In the +link{monthView, month} view,
// multiple instances of this component are created and shown beneath the headers
// for each week.
// <p>
// Users may click in this component to create an <i>all-day</i> event on the clicked date, or
// drag horizontally to create an event that spans multiple full days - either shows the new
// event for editing in the usual +link{calendar.eventEditor}.
// <p>
// When an event spans multiple days, or is marked as an all-day event, its associated
// +link{calendar.eventCanvas, eventCanvas} is displayed in this
// component instead of in the main body of a given +link{class:CalendarView}.  Each such event
// appears as a narrow horizontal bar spanning the full width of the columns its
// +link{calendar.startDateField, start} and +link{calendar.endDateField, end} dates involve.
// In the <i>Month-view</i>, if an event spans multiple weeks, the longEvents component in each
// week will show this event.
// <p>
// If a multi-day event spans beyond the current view's date-range at one or both ends, those
// ends are styled to indicate the event continues in that direction.  Otherwise, if an event
// can be edited, users may drag -move or -resize it, or click it to show the
// <i>eventEditor</i> window.
// <p>
// If multiple long-events occupy the same date-range, they are stacked vertically and this
// component will resize to accommodate them.
// <p>
// This component is an +link{type:AutoChild} and as such may be customized via
// <code>calendarView.longEventsLayoutProperties</code> and
// <code>calendarView.longEventsLayoutDefaults</code>.
//
// @group calendarEvent
// @visibility internal
// @see CalendarEvent
...
// with them in normal Calendar views.   users need a clear and means of viewing and interacting
// with those events
//
//<
