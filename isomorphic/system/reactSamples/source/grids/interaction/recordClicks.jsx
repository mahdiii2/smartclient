...
import { SC, DVField, DetailViewer, DetailViewerField, LGField, ListGrid } from 'smartclient-eval/react';

// a "render target" can be defined to allow JSX to be loaded dynamically into an existing application.  This Showcase sets a
// render target of "showcaseSample" so each JSX file can be loaded and later removed when the user is done with the sample.
let target = SC.getRenderTarget() || "showcaseSample";

let onCountryListRecordClick = function(viewer,record,recordNum,field,fieldNum,value,rawValue,editedRecord) {return countryDetails.setData(record)};
let onCountryListRecordDoubleClick = function(viewer,record,recordNum,field,fieldNum,value,rawValue,editedRecord) {return isc.say('Double-clicked country: <b>'+record.countryName+'</b>')};
let onCountryListRowContextClick = function(record,rowNum,colNum) {isc.say('Context-clicked country: <b>'+record.countryName+'</b>'); return false;};
...
SC.render(
    <>
        <ListGrid ID="countryList" data={countryData} width="500" height="224" recordClick={onCountryListRecordClick} recordDoubleClick={onCountryListRecordDoubleClick} rowContextClick={onCountryListRowContextClick}>
            <fields>
        ...
        <DetailViewer ID="countryDetails" emptyMessage="click a row in the grid" width="500" top="250">
            <fields>
                <DVField name="countryName" title="Country"/>
                <DVField name="countryCode" title="Code"/>
                <DVField name="government" title="Government"/>
                <DVField name="article" type="link" title="More Information" linkText="Click here for more information"/>
            </fields>
        </DetailViewer>
    </>,
