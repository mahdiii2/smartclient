...
let onCategoryGetGridSummary = function (records, summaryField) {
    var seenCategories = {};
...
let onCategorySummaryFunction = function (records, summaryField) {
    var seenCategories = {};
    for (var i = 0; i < records.length; i++) {
        seenCategories[records[i].category] = true;
    }
...
let onUnitPriceFormatCellValue = function (value, record, rowNum, colNum, grid) {
    if (record == null || record._isGroup) {
        return '&nbsp;';
    } else {
        return isc.NumberUtil.format(value, '\xA4,0.00');
    }
};
...
SC.render(
    <>
        ...
        <ListGrid ID="companyList" dataSource="orderItemLocalDS" showAllRecords="true" canEdit="true" groupByField="category" autoFetchData="true" showGridSummary="true" showGroupSummary="true" width="815" height="520" groupStartOpen="all" showGroupSummaryInHeader="true">
            <fields>
                <LGField name="orderID" summaryFunction="count" includeInRecordSummary="false"/>
                <LGField name="itemDescription"/>
                <LGField name="category" summaryFunction={onCategorySummaryFunction} showGridSummary="true" getGridSummary={onCategoryGetGridSummary}/>
                <LGField name="shipDate" summaryFunction="max" showGridSummary="false" showGroupSummary="true"/>
                <LGField name="quantity" showGridSummary="false" showGroupSummary="false"/>
                <LGField name="unitPrice" showGridSummary="false" showGroupSummary="false" formatCellValue={onUnitPriceFormatCellValue}/>
                <LGField name="Total" type="summary" summaryFunction="sum" showGridSummary="true" showGroupSummary="true" align="right" format="¤,0.00" recordSummaryFunction="multiplier"/>
            </fields>
        </ListGrid>
    </>,
