...
else isc._preLog=[isc._pTM]}isc.definingFramework=true;isc.defineClass("DocCookieState");
isc.A=isc.DocCookieState.getPrototype();
isc.B=isc._allFuncs;
isc.C=isc.B._maxIndex;
isc.D=isc._funcClasses;
isc.D[isc.C]=isc.A.Class;
isc.B.push(isc.A.init=function isc_DocCookieState_init(){
...
,isc.A.__getDefaultData=function isc_DocCookieState___getDefaultData(){
    return isc.clone(this.defaultData||{});
}
,isc.A.pageLoaded=function isc_DocCookieState_pageLoaded(){
    this.load();
    this.processCallback();
}
,isc.A.processCallback=function isc_DocCookieState_processCallback(){
    if(this.onload){
        this.fireCallback(this.onload,["state","data"],[this,this.data]);
    }
}
,isc.A.load=function isc_DocCookieState_load(){
    ...
    if(!this.data)this.data=this.__getDefaultData();
    return this.data;
}
,isc.A.canUpdate=function isc_DocCookieState_canUpdate(){
    return isc.Page.isLoaded();
}
,isc.A.store=function isc_DocCookieState_store(data){
    if(!this.canUpdate())return false;
    if(!data&&!this.data)return true;
    if(data)this.data=data;
    if(this.disableCookieStore)return false;
    var stateString=isc.Comm.serialize(this.data,false);
    this.logDebug("storing: "+stateString+" - length: "+stateString.length);
    return isc.Cookie.set(this.cookieName,stateString,this.cookiePath,this.cookieDomain,this.cookieExpiration);
}
,isc.A.add=function isc_DocCookieState_add(data){
    if(!this.canUpdate())return false;
    if(!data)return true;
    if(!this.data)this.data=this.__getDefaultData();
    isc.addPropertiesWithAssign(this.data,data);
    return this.store();
}
,isc.A.clear=function isc_DocCookieState_clear(){
    return this.reset();
}
,isc.A.reset=function isc_DocCookieState_reset(){
    if(!this.canUpdate())return;
    this.data=this.__getDefaultData();
    this.store();
}
,isc.A.getStoredSize=function isc_DocCookieState_getStoredSize(data){
    if(!data)data=this.data;
    if(!data)return 0;
    return isc.Comm.serialize(data,false).length;
}
);
...
,isc.A.keyPress=function isc_DocGrid_keyPress(event,eventInfo){
    ...
    if(isc.EH.metaKeyDown()||isc.EH.ctrlKeyDown())return;
    var startIndex=0;
...
isc.A=isc.DocUtils;
isc.A.docHoverStyle="docHover";
isc.A.docLinkStyle="docLink";
isc.A.docMethodStyle="docMethod";
isc.A.docAttributeStyle="docAttribute";
isc.A.docParameterStyle="docParameter";
isc.A.docOptionalParameterStyle="docOptionalParameter";
...
,isc.A.linkForRef=function isc_c_DocUtils_linkForRef(ref,name,linkRef,skipParens,baseTypeMode){
    ...
    return isc.StringBuffer.concat("<a class='"+isc.DocUtils.docLinkStyle+"' href='"+href+"' ",
...
,isc.A._showDocHover=function isc_c_DocUtils__showDocHover(ref){
    ...
    if(html){
        var hoverCanvasProperties={
            width:500,
            baseStyle:isc.DocUtils.docHoverStyle,
            forceShow:true
        };
...
,isc.A.linkForDocNode=function isc_c_DocUtils_linkForDocNode(id,name){
    ...
    return isc.StringBuffer.concat("<a class='"+isc.DocUtils.docLinkStyle+"' href='' ",
...
,isc.A.hoverHTML=function isc_c_JSDoc_hoverHTML(container,item,linkName,omitAttrs){
    ...
    if(docItem&&(omitAttrs!=null&&omitAttrs.length>0)){
        if(this.dataIsXML){
            docItem=docItem.cloneNode();
            for(var i=0;i<omitAttrs.length;i++){
                docItem.removeAttribute(omitAttrs[i]);
            }
        }else{
...
,isc.A.sourceLinkOver=function isc_c_JSDoc_sourceLinkOver(){
    if(!isc.Hover._hoverHasFocus)isc.Hover.setAction(isc.DocUtils,isc.DocUtils.doSourceHover,null,300);
}
,isc.A.sourceLinkOut=function isc_c_JSDoc_sourceLinkOut(){
    if(!isc.Hover._hoverHasFocus)isc.Hover.clear();
}
,isc.A.refLinkOver=function isc_c_JSDoc_refLinkOver(ref){
    if(!isc.Hover._hoverHasFocus)isc.DocUtils._showDocHover(ref);
}
,isc.A.refLinkOut=function isc_c_JSDoc_refLinkOut(){
    if(!isc.Hover._hoverHasFocus)isc.Hover.clear();
}
,isc.A.refLinkClick=function isc_c_JSDoc_refLinkClick(ref,linkRef){
...
,isc.A.getAllDocItemsByType=function isc_c_JSDoc_getAllDocItemsByType(type){
...
,isc.A.getActions=function isc_c_JSDoc_getActions(className){
    var ds=isc.DS.get(className);
    if(ds==null)return null;
    var actions,
        seenActions={};
...
,isc.A.getEvents=function isc_c_JSDoc_getEvents(className,showSuperClassEvents,basicOnly){
    var ds=isc.DS.get(className);
    if(ds==null)return null;
    var events,
        seenEvents={};
    var classObj=isc.ClassFactory.getClass(className);
    if(classObj&&classObj._stringMethodRegistry&&
        !isc.isAn.emptyObject(classObj._stringMethodRegistry))
    {
        var chain=this.getInheritanceChain(classObj,ds,showSuperClassEvents),
            classMethods,
            superclassMethods=[],
            newMethods
        ;
        for(var i=0;i<chain.length;i++){
            ...
            for(var j=0;j<newMethods.length;j++){
                ...
                if(!docItem){
                    var dsMethod=(ds.methods?ds.methods.find("name",methodName):null);
                    if(!dsMethod){
                ...
                var docData=docItem?isc.jsdoc.toJS(docItem):dsMethod;
                if(events==null)events=[];
                var event=isc.addProperties({},docData,dsMethod);
                events.add(event);
            }
        }
        if(!basicOnly)return events;
    }
    while(ds!=null){
        var dsMethods=ds.methods;
        if(dsMethods==null){
            if(ds.showSuperClassEvents==false)break;
            ds=ds.superDS();
            continue;
        }
        for(var i=0;i<dsMethods.length;i++){
            var dsMethod=dsMethods[i];
            if((basicOnly&&!dsMethod.basic)||dsMethod.action)continue;
            if(dsMethod.name in seenEvents)continue;
            seenEvents[dsMethod.name]=dsMethod;
            if(dsMethod.inapplicable==true||dsMethod.inapplicable=="true")continue;
            var docItem=isc.jsdoc.getDocItem("method:"+ds.ID+"."+dsMethod.name),
                docData=docItem?isc.jsdoc.toJS(docItem):dsMethod;
            if(events==null)events=[];
            var event=isc.addProperties({},docData,dsMethod);
            events.add(event);
            var params=event.params;
            if(params!=null&&!isc.isAn.Array(params))event.params=[params];
        }
        if(ds.showSuperClassActions==false)break;
        ds=ds.superDS();
    }
...
,isc.A.getDefaultEvent=function isc_c_JSDoc_getDefaultEvent(className){
    var events=this.getEvents(className,true,true),
        defaultEvent
    ;
    if(events)defaultEvent=events.find("default",true);
    return defaultEvent;
}
,isc.A.getInheritanceChain=function isc_c_JSDoc_getInheritanceChain(classObj,dataSource,showSuperClassEvents){
    var chain=[],
        showSuper=dataSource.showSuperClassEvents||showSuperClassEvents
    ;
    if(showSuper&&
...
isc.D=isc._funcClasses;
isc.D[isc.C]=isc.A.Class;
isc.B.push(isc.A.generateHTML=function isc_DetailFormatter_generateHTML(context,accum,separate){
    if(!accum)accum=isc.StringBuffer.create();
...
,isc.A.generate_values=function isc_DetailFormatter_generate_values(context,accum){
    accum.append("<b>Valid values:</b><p><table cellpadding=2 >");
...
,isc.A.attrHTML=function isc_c_AttrFormatter_attrHTML(attr,accum,linkName){
    ...
    if(this._htmlTemplate==null){
        this._htmlTemplate=[
            "<table BORDER='0' CELLSPACING='0' WIDTH='100%' ><tr><td><b>",
            null,
            "</b>",
            null,
...
,isc.A.formatDefaultValue=function isc_c_AttrFormatter_formatDefaultValue(attr){
    ...
    if(defaultValue){
        ...
        var prelim=attr.constant?" (Constant), <i>value</i>: ":", <i>defaultValue</i>: ";
        if(isc.Browser.hasTextOverflowEllipsis){
            defaultValue=prelim+
                "<a style='display:inline-block;white-space:nowrap;max-width:400px;overflow:hidden;"+
                        isc.Browser._textOverflowPropertyName+":ellipsis;vertical-align:middle' "+
                    "title='"+defaultValue.replaceAll("'","&#39;")+"'>"+
                defaultValue+
                "</a>";
        }else{
            defaultValue=prelim+defaultValue;
        }
...
,isc.A.contents=function isc_AttrFormatter_contents(){
    ...
    if(isc.AttrFormatter._attrStart==null){
        isc.AttrFormatter._attrStart=[
            ...
            "<tr BGCOLOR='lightgrey' ><td colspan='2'>Group: <b>",
            null,
            "</b></td></tr>"
        ];
    }
    ...
    if(method.params){
        ...
        for(var k=0;k<method.params.length;k++){
            ...
            if(param.optional!=null&&param.optional.toString()=="true"&&!inOptional){
                inOptional=true;
                methodParams.append("<span class='"+isc.DocUtils.docOptionalParameterStyle+"'>[");
            }
...
,isc.A.search=function isc_DocSearchBar_search(searchString){
    ...
    var refs=isc.jsdoc&&isc.jsdoc.getRefs(),
        numRefs=!refs?0:refs.getLength();
    for(var k=0;k<numRefs;++k){
        var ref=refs[k];
        var m,score=0;
        ...
        if(score!=0){
            ...
            var title=isc.jsdoc.getAttribute(item,"title");
            var type=isc.jsdoc.getAttribute(item,"type");
            var rawGroups=isc.jsdoc.getAttribute(item,"groups"),
                groups=rawGroups&&rawGroups.map(function(rawGroup){
                    return isc.DocUtils.linkForRef("group:"+rawGroup);
                });
...
isc.D=isc._funcClasses;
isc.D[isc.C]=isc.A.Class;
isc.A.searchResultsTabTitle="&nbsp;Search Results&nbsp;";
isc.A.noResultsPaneConstructor="Canvas";
isc.A.noResultsPaneDefaults={
        contents:"<br><br>&nbsp;&nbsp;<b>No matches.</b>"
    };
isc.A.resultsPaneConstructor="VLayout";
isc.A.searchPreviewConstructor="Canvas";
isc.A.searchPreviewDefaults={
        canSelectText:true,
...
isc.A.searchDSConstructor="DataSource";
isc.A.searchDSDefaults={
        clientOnly:true,
        fields:[
            {name:"score",title:"Score",width:80,format:"0.#"},
            {name:"name",canGroupBy:false,title:"Name",width:"*"},
            {name:"title",hidden:true},
            {name:"ref",primaryKey:true,title:"Match",width:"*",showIf:"false"},
            {name:"type",title:"Type",width:80,valueMap:["group","class","interface","object","type","classAttr","classMethod","staticMethod","attr","method"]},
            {name:"groups",title:"Group",width:100,multiple:true}
        ]
    };
isc.A.filterFormConstructor="DocFilterField";
isc.A.filterFormDefaults={
        fieldName:"name"
    };
isc.A.searchResultsGridConstructor="DocGrid";
isc.A.searchResultsGridDefaults={
        height:300,
        initialSort:[{
            property:"score",
        ...
        defaultFields:[{
            ...
            getGroupTitle:function(groupValue,groupNode,field,fieldName,grid){
                var groupMembers=groupNode.groupMembers,
                    numGroupMembers=!groupMembers?0:groupMembers.length;
                return groupValue+" ("+numGroupMembers+")";
            }
        },{
            name:"groups",
            sortDirection:"ascending",
            canGroupBy:true,
            _getDocGroupTitle:function(docGroup){
                var docItem=isc.jsdoc.getDocItem("group:"+docGroup);
                return docItem&&docItem.title||docGroup;
            },
            _getFirstDocGroupTitle:function(hit){
                if(!hit||!isc.isAn.Array(hit.rawGroups))return"";
                var docGroup=hit.rawGroups[0];
                return this._getDocGroupTitle(docGroup);
            },
            sortNormalizer:function(hit,fieldName,context){
                if(!hit||!isc.isAn.Array(hit.rawGroups))return"";
                var field=context.getFieldByName(fieldName),
                    docGroupTitles=hit.rawGroups.map(field._getDocGroupTitle,field);
                return docGroupTitles.join(",");
            },
            groupSortNormalizer:function(hit,fieldName,context){
                var field=context.getFieldByName(fieldName)
                return field._getFirstDocGroupTitle(hit);
            },
            getGroupValue:function(value,hit,field,fieldName,grid){
                return field._getFirstDocGroupTitle(hit);
            },
            getGroupTitle:function(groupValue,groupNode,field,fieldName,grid){
                var groupMembers=groupNode.groupMembers,
...
isc.A.searchBarConstructor="DocSearchBar";
isc.A.searchBarDefaults={
        height:20
    };
isc.B.push(isc.A.initWidget=function isc_DocSearchResults_initWidget(){
        ...
        if(this.hits.length==0){
            this.addTab({
                title:this.searchResultsTabTitle,
                pane:this.createAutoChild("noResultsPane")
            });
            return;
        }
        var searchPreview=this.searchPreview=this.createAutoChild("searchPreview");
        var filterForm=this.createAutoChild("filterForm");
        var searchDS=this.createAutoChild("searchDS");
        var searchResultsGrid=this.searchResultsGrid=this.createAutoChild("searchResultsGrid",{
            dataSource:searchDS,
            docPreview:searchPreview,
        ...
        this.searchBar=this.createAutoChild("searchBar",{
        ...
        this.addTab({
            title:this.searchResultsTabTitle,
            pane:this.createAutoChild("resultsPane",{
                members:[filterForm,searchResultsGrid,searchPreview]
            })
        });
    }
);
...
isc.B.push(isc.A.getHTML=function isc_c_GroupViewer_getHTML(group,linkName){
    if(group.description==null&&(group.refs==null||group.refs.length==0)){
        return null;
    }
    var accum=isc.StringBuffer.create();
    ...
    accum.append("<table ><tr><td>&nbsp;</td><td>",
                "<font size='+1'>",this.prefix,"<b>",
                linkName?isc.DocUtils.linkForRef(group.ref):title,"</b></font><br><p>",
                 (group.description?isc.DocUtils.stripHTML(group.description):"")
                );
    isc.DetailFormatter.newInstance({
        detailItems:isc.GroupViewer.detailItems
    }).generateHTML(group,accum,true);
    var refs=group.refs;
    if(!isc.isAn.Array(refs))refs=refs?[refs]:[];
    if(refs&&refs.length!=0){
        accum.append("<br><br><b>Methods and Properties referencing group ",title,
                     " </b><br><br>");
...
,isc.A.hoverHTML=function isc_c_ClassViewer_hoverHTML(classDoc,linkName){
    if(this._classDescriptionTemplate==null){
        this._classDescriptionTemplate=[
            "<table BORDER='0' WIDTH='100%'><tr><td width='1'></td><td>",
    ...
    if(classDoc.groups&&classDoc.groups.contains("experimental")){
        cdt[10]="<b><i><span style='color:red'>Note: this is currently an experimental "+
            "feature and not covered by normal support guarantees. See "+
            isc.DocUtils.linkForRef("group:experimental","Experimental Features")+
            " for more information.</span></i></b><br><br>";
    }else{
        cdt[10]=null;
    }
    cdt[11]=classDoc.description
        ?isc.DocUtils.stripHTML(classDoc.description)
        :isc.emptyString;
    if(classDoc.deprecated){
        cdt[12]="<br><br><B>DEPRECATED:&nbsp;"+classDoc.deprecated+"</B><br>";
    }else{
        cdt[12]=null;
    }
    cdt[14]=isc.DetailFormatter.newInstance({
            detailItems:this.detailItems
...
,isc.A.getOverrideProperties=function isc_c_ClassViewer_getOverrideProperties(className){
    ...
    for(var propName in proto){
        ...
        if(method==isc.Class.NO_OP)continue;
        ...
        if(implementingSuper!=null){
            ...
            if(superClass._stringMethodRegistry[propName]!=null){
                var ref="method:"+superClass.getClassName()+"."+propName;
                var docItem=isc.jsdoc.getDocItem(ref);
                if(docItem&&!isc.jsdoc.getAttribute(docItem,"override")){
                    overriddenProperties.add(ref);
                    overrideProperties.add(refStart+propName);
...
,isc.A.tabSelected=function isc_ClassViewer_tabSelected(tabNum,pane,ID){
    ...
    var state=this.docViewer.state;
    if(id!=null&&isc.isAn.Object(state.data)){
        ...
        if(!state.store()){
            var tabs={};
            if(tabNum!=0)tabs[id]=tabNum;
            state.add({
                op:this.docTree._getCurrentlyOpenPaths(),
                tabs:tabs
            });
        }
    }
...
,isc.A.initOverviewPane=function isc_ClassViewer_initOverviewPane(tabSet,tabID){
    ...
    if(this.hasInstanceAPIs){
        descriptionText+="<br>&nbsp;&nbsp;<b><a class='"+isc.DocUtils.docLinkStyle+"' "+
            "href='' onclick='"+this.getID()+
    ...
    if(this.hasClassAPIs||this.hasStaticAPIs){
        ...
        descriptionText+="<br>&nbsp;&nbsp;<b><a class='"+isc.DocUtils.docLinkStyle+"' href='' onclick='"+this.getID()+
...
,isc.A.initAPIPane=function isc_ClassViewer_initAPIPane(tabSet,type,tabID){
    ...
    var apiDS=isc.DataSource.create({
        ...
        fields:[
            ...
            {name:"isMethod",type:"boolean",required:false,hidden:true},
            {name:"returns",type:"custom",required:false,hidden:true},
            {name:"valueType",title:"Type/Return Type",type:"summary",width:120,
                ...
                getRecordSummaryDependencies:function(fields,summaryField){
                    return["isMethod","returns"];
                },
                formatCellValue:function(value,record){
            ...
            {name:"name",canGroupBy:false,primaryKey:true,title:"Name",width:"*",
                ...
                formatCellValue:function(value,record){
                    ...
                    if(record.isMethod){
                        formattedValue="<span class='"+isc.DocUtils.docMethodStyle+"'>"+
                            value+"&nbsp;<span class='"+isc.DocUtils.docParameterStyle+"'>"+
                            isc.MethodFormatter.formatMethodParams(record)+"</span></span>";
                    }else{
                        formattedValue="<span class='"+isc.DocUtils.docAttributeStyle+"'>"+
                            value+"</span>";
                    }
                    if(isc.renderForBot){
                        return"<a class='"+isc.DocUtils.docLinkStyle+
                            "' href='?id="+isc.DocViewer.instance.encodeHistoryID(record.ref)+
                            "'>"+formattedValue+"</a>";
                    }
...
,isc.A._autoGenOverrideDocs=function isc_ClassViewer__autoGenOverrideDocs(){
    if(this.docItem.type!="class")return;
...
,isc.A._buildSummaryView=function isc_SummaryViewer__buildSummaryView(){
    ...
    for(var i=0;i<childNodes.length;i++){
        ...
        if(node.ref){
            ...
            accum.append("<a class='"+isc.DocUtils.docLinkStyle+"' href='' onclick='",this.docViewer.getID(),"._showNode(\"",
...
,isc.A._doSelect=function isc_DocViewer__doSelect(classViewer,ref,viewRef,maintainCurrentTab){
    ...
    if(!classViewer.isDrawn()||classViewer[type+"Grid"]==null){
...
,isc.A._buildLeftPane=function isc_DocViewer__buildLeftPane(){
    ...
    this._docTree.addProperties({
        docViewer:this,
        _getCurrentlyOpenPaths:function(){
        ...
        changeDataVisibility:function(node,newState){
            ...
            if(!openPaths){
                openPaths=this._getCurrentlyOpenPaths();
            }else{
                var path=this.getPath(node);
                if(newState){
                    if(!openPaths.contains(path))openPaths.add(path);
                }else{
                    while(openPaths.remove(path)){
                    }
                }
            }
            if(!this.docViewer.state.add({op:openPaths})){
                this.docViewer.state.add({
                    op:this._getCurrentlyOpenPaths(),
                    tabs:null
                });
            }
        }
