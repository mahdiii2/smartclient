...
isc.Window.addProperties({
    ...
    doVoiceCommand : function (command) {
        ...
        // lower-cased command - if so, click that widget and return
        ...
        if (isc.AI && isc.AI.allowVoiceAssist) {
    ...
    getDropComponent : function (dragTarget, dropPosition) {
        ...
        if (dropComponent) {
            ...
            if (!isc.isA.Portlet(dropComponent)) {
                dropComponent = this.portalLayout._createPortlet(dropComponent);
            }
        }
...
isc.defineClass("PortalLayout", "Layout").addProperties({
    ...
    // Override getDropComponent to intercept Portlet drop directly on the PortalLayout
    ...
    getDropComponent : function (dragTarget, dropPosition) {
        ...
        if (this.canAddColumns && dragTarget.dragType != "PortalColumn") {
            this.addColumnOnDrop(dragTarget, dropPosition);
            return false;
        }
    ...
    addColumnOnDrop : function (dragTarget, dropPosition) {
        ...
        // ignore the drop attempt.
        if (isc.isA.PortalColumn(dragTarget)) return false;
        ...
        if (this.contains(dropComponent)) {
            var row = dropComponent.parentElement,
                currentColumn = isc.isA.PortalRow(row) ? row.creator : null,
                currentColNum = this.members.indexOf(currentColumn);
            if (currentColNum == dropPosition || currentColNum == dropPosition-1) {
                portlets = currentColumn.getPortlets();
        ...
        column = this.getMember(dropPosition);
        column.addPortlet(dropComponent);
    },
    ...
    // @param [rowNum] (int) indicates the row number being dropped on. If the <code>rowNum</code>
    //  is null, a new column will be created to contain the portlet.
    // @param [dropPosition] (int) Drop position within an existing row. If the <code>dropPosition</code>
    //  is null, a new row will be created to contain the portlet.
