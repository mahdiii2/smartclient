...
var transformRules = {
    ...
    classTranslations: {
        ...
        "com.balsamiq.mockups::ButtonBar": "ToolStrip",
    ...
    iconsMap: {
        "DownloadIcon": "/actions/download.png",
    ...
    widgetPropertyTranslations: {
        ...
        "com.balsamiq.mockups::BrowserWindow" : {
            ...
            onProcessFinished : function (control) {
                var undef;
                if (control.top != undef) control.top = parseInt(control.top) + 71;
                if (control.height != "100%" && control.height != "*") control.height = parseInt(control.height) - 77;                
            }
        },
        ...
        "com.balsamiq.mockups::Button": {
            ...
            controlPropertiesParser : function (name, value) {
                if (name == "icon") {
                    var src = value.name || value.split("|")[0];
                    if (transformRules.iconsMap[src]) {
                        return "[SKIN]" + transformRules.iconsMap[src];    
                    } else {
                        return "[SKIN]/actions/" + src;    
                    }                    
                }
                return value;
            },
        ...
        "com.balsamiq.mockups::Image": {
            "src": "src",
            afterInit : function (name, control) {
                if (control.src != null && isc.isA.String(control.src) && !control.src.trim().startsWith("http://")) {
            ...
            mapAsset : function (name, control, assets) {
                if (control.src != null && control.src.ID) {
                    var ID = control.src.ID;
                    for (var i = 0; i < assets.length; i++) {
                        var asset = assets[i];
                        if (asset.ID == ID) {
                            var attributes = asset.attributes,
                                fileName = attributes.importedFrom
                            ;
        ...
        "com.balsamiq.mockups::Component": {
            src: "symbolPath",
            override: "override",   // BMML version
            overrides: "overrides"  // BMPR version
        },
