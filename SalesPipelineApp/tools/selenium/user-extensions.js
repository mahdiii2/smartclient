...
PageBot.prototype.getAutWindow = function() {
    ...
    // if SmartClient isn't loaded on the target window, just bail; require
    // AutoTest, and wait for page load to ensure other modules are loaded
    if (!autWindow.isc || !autWindow.isc.AutoTest || !autWindow.isc.Page.isLoaded()) return;
    ...
    if (this.isSCLocator(locator)) {
        ...
        if (isSCTarget) {
            ...
            if (isc.isA.GridRenderer(canvas)) {
                ...
                // if target is GridRenderer, bias drop coordinates downward within the DOM
                // element to ensure deterministic behavior when dropping on row, unless drop
                // position is "after", since we don't want to move off bottom of the element
                } else if (!targetLocator.match(/\/after$/i)) {
                    coords[1] += Math.ceil(dropElement.offsetHeight / 8);
                }
            }
...
Selenium.prototype.isElementClickable = function (locator) {
    ...
    var element = this.page().findElement(locator),
        autWindow = this.getAutWindow(),
        autotest = autWindow.isc.AutoTest
    ;
    this.hookWindowOpenForSelectWindow(autWindow);
...
Selenium.prototype.isGridDone = function (locator, includeEdits) {
    ...
    // tranform includeEdits into a boolean from Selenium string
    includeEdits = includeEdits == "true" || includeEdits == "yes";

    return autotest.seleniumExecute("isGridDone", element, locator, includeEdits);
};
...
Selenium.prototype.isSystemDone = function (includeRedraws, includeEdits) {

    LOG.info("Located in isScSystemDone");

    var autotest = this.getAutWindow().isc.AutoTest;

    // convert includeRedraws/includeEdits to booleans for seleniumExecute()
    includeRedraws = includeRedraws == "true" || includeRedraws == "yes";
    includeEdits   = includeEdits   == "true" || includeEdits   == "yes";

    return autotest.seleniumExecute("isSystemDone", includeRedraws, includeEdits);
};
...
Selenium.prototype.doExampleOpen = function(showcase, mappedId) {
    ...
    return this.doOpen(url + "?AI=true#" + mappedId);
};
...
Selenium.prototype.doFocus = function (locator, clearModifiers) {
    ...
    if (this.hasSC()) {
        // put focus into canvas and/or item
        var isc = this.getAutWindow().isc;        
        isc.AutoTest.locatorFocus(locator);
    } else {
...
// fix issue where Selenium can only remember one window created without a name
Selenium.prototype.hookWindowOpenForSelectWindow  = function (autWindow) {
    if (autWindow._openWindowHooked) return;
    autWindow._openWindowHooked = true;

    var selenium = this;

    var oldOpenWindow = autWindow.open;
    autWindow.open = function () {
        var newWindow = oldOpenWindow.apply(autWindow, arguments);

        // correct Selenium storage for windows without assigned name
        var undef, openedWindows = selenium.browserbot.openedWindows,
            undefinedWindow = openedWindows[undef];
        if (undefinedWindow) {
            openedWindows["_isc_" + (new Date()).getTime()] = undefinedWindow;
            delete openedWindows[undef];
        };

        selenium.hookWindowOpenForSelectWindow(newWindow);
        return newWindow;
    };

};

// convenience method to support waitForAndSelectWindow()
Selenium.prototype.isAndSelectWindow = function (locator) {
    try {
        this.doSelectWindow(locator);
        // Work around a Selenium IDE bug that causes the browserbot not to target the selected
        // window, even if doSelectWindow() has successfully completed (hitting no exception).
        // We know it's a Selenium bug because even if you never call doSelectWindow() until
        // the new window shows up in browserbot's table with your desired title, even still
        // calling doSelectWindow() will sometimes not properly target that new window for
        // future waitForElementXXX calls by browserbot.
                this.doPause(500);
        this.doSelectWindow();
        this.doSelectWindow(locator);
        ...
        LOG.debug("Unable to find window with locator: " + locator);
        return false;
    }
}
